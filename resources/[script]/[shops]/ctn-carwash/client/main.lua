                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], '~g~E~w~ - Car Wash (€'..Config.DefaultPrice..')')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('ctn-carwash:server:washCar')
                                end
                            else
...
RegisterNetEvent('ctn-carwash:client:washCar')
AddEventHandler('ctn-carwash:client:washCar', function()
    local PlayerPed = GetPlayerPed(-1)
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    QBCore.Functions.Progressbar("search_cabin", "Car wash..", math.random(4000, 8000), false, true, {
