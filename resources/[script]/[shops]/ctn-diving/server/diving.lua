...
QBCore.Functions.CreateCallback('ctn-diving:server:GetDivingConfig', function(source, cb)
    cb(QBDiving.Locations, CurrentDivingArea)
end)

RegisterServerEvent('ctn-diving:server:TakeCoral')
AddEventHandler('ctn-diving:server:TakeCoral', function(Area, Coral, Bool)
    local src = source
    ...
    if (QBDiving.Locations[Area].TotalCoral - 1) == 0 then
        ...
        TriggerClientEvent('ctn-diving:client:NewLocations', -1)
    else
    ...
    TriggerClientEvent('ctn-diving:server:UpdateCoral', -1, Area, Coral, Bool)
end)

RegisterServerEvent('ctn-diving:server:RemoveGear')
AddEventHandler('ctn-diving:server:RemoveGear', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-diving:server:GiveBackGear')
AddEventHandler('ctn-diving:server:GiveBackGear', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
