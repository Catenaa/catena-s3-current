...
RegisterServerEvent('ctn-diving:server:SetBerthVehicle')
AddEventHandler('ctn-diving:server:SetBerthVehicle', function(BerthId, vehicleModel)
    TriggerClientEvent('ctn-diving:client:SetBerthVehicle', -1, BerthId, vehicleModel)
    
    QBBoatshop.Locations["berths"][BerthId]["boatModel"] = boatModel
end)

RegisterServerEvent('ctn-diving:server:SetDockInUse')
AddEventHandler('ctn-diving:server:SetDockInUse', function(BerthId, InUse)
    QBBoatshop.Locations["berths"][BerthId]["inUse"] = InUse
    TriggerClientEvent('ctn-diving:client:SetDockInUse', -1, BerthId, InUse)
end)

QBCore.Functions.CreateCallback('ctn-diving:server:GetBusyDocks', function(source, cb)
    cb(QBBoatshop.Locations["berths"])
end)

RegisterServerEvent('ctn-diving:server:BuyBoat')
AddEventHandler('ctn-diving:server:BuyBoat', function(boatModel, BerthId)
    ...
    local plate = "CTN"..math.random(1111, 9999)

    if PlayerMoney.cash >= BoatPrice then
        Player.Functions.RemoveMoney('cash', BoatPrice, "bought-boat")
        TriggerClientEvent('ctn-diving:client:BuyBoat', src, boatModel, plate)
...
QBCore.Functions.CreateUseableItem("jerry_can", function(source, item)
    ...
    TriggerClientEvent("ctn-diving:client:UseJerrycan", source)
end)

QBCore.Functions.CreateUseableItem("diving_gear", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)

    TriggerClientEvent("ctn-diving:client:UseGear", source, true)
end)

RegisterServerEvent('ctn-diving:server:RemoveItem')
AddEventHandler('ctn-diving:server:RemoveItem', function(item, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-diving:server:GetMyBoats', function(source, cb, dock)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-diving:server:GetDepotBoats', function(source, cb, dock)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-diving:server:SetBoatState')
AddEventHandler('ctn-diving:server:SetBoatState', function(plate, state, boathouse)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-diving:server:CallCops')
AddEventHandler('ctn-diving:server:CallCops', function(Coords)
    local src = source
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                ...
                TriggerClientEvent('ctn-diving:client:CallCops', Player.PlayerData.source, Coords, msg)
                local alertData = {
...
QBCore.Commands.Add("divremove", "Take off your diving gear", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent("ctn-diving:client:UseGear", source, false)
end)

RegisterServerEvent('ctn-diving:server:SellCoral')
AddEventHandler('ctn-diving:server:SellCoral', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if HasCoral(src) then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'You don\'t have coral to sell..', 'error')
    end
end)
