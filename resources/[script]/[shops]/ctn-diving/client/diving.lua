...
RegisterNetEvent('ctn-diving:client:NewLocations')
AddEventHandler('ctn-diving:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('ctn-diving:server:GetDivingConfig', function(Config, Area)
        QBDiving.Locations = Config
        TriggerEvent('ctn-diving:client:SetDivingLocation', Area)
    end)
end)

RegisterNetEvent('ctn-diving:client:SetDivingLocation')
AddEventHandler('ctn-diving:client:SetDivingLocation', function(DivingLocation)
...
function TakeCoral(coral)
    ...
    TriggerServerEvent('ctn-diving:server:TakeCoral', CurrentDivingLocation.Area, coral, true)
end

RegisterNetEvent('ctn-diving:client:UpdateCoral')
AddEventHandler('ctn-diving:client:UpdateCoral', function(Area, Coral, Bool)
...
function CallCops()
    ...
    if Call == Chance then
        TriggerServerEvent('ctn-diving:server:CallCops', Coords)
    end
end

RegisterNetEvent('ctn-diving:server:CallCops')
AddEventHandler('ctn-diving:server:CallCops', function(Coords, msg)
...
RegisterNetEvent('ctn-diving:client:UseGear')
AddEventHandler('ctn-diving:client:UseGear', function(bool)
    if bool then
        GearAnim()
        QBCore.Functions.Progressbar("equip_gear", "Put on a diving suit..", 5000, false, true, {}, {}, {}, {}, function() -- Done
            ...
            TriggerServerEvent('ctn-diving:server:RemoveGear')
            ClearPedTasks(GetPlayerPed(-1))
            TriggerEvent('chatMessage', "SYSTEM", "error", "/divremove to take off your diving gear!")
        end)
    else
        if currentGear.enabled then
            GearAnim()
            QBCore.Functions.Progressbar("remove_gear", "Take off your diving gear..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                DeleteGear()

                SetEnableScuba(GetPlayerPed(-1), false)
                SetPedMaxTimeUnderwater(GetPlayerPed(-1), 1.00)
                currentGear.enabled = false
                TriggerServerEvent('ctn-diving:server:GiveBackGear')
                ClearPedTasks(GetPlayerPed(-1))
