            ...
            if BuyDistance < 2 then                
                ...
                if not Buying then
                    ...
                    if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustReleased(0, Keys["7"]) then
                        TriggerServerEvent('ctn-diving:server:BuyBoat', QBBoatshop.Locations["berths"][ClosestBerth]["boatModel"], ClosestBerth)
                        Buying = false
...
RegisterNetEvent('ctn-diving:client:BuyBoat')
AddEventHandler('ctn-diving:client:BuyBoat', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
