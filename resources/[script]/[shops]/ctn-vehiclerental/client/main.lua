...
RentVehicle = function(selectedVehicle)
    ...
    if IsPedInAnyVehicle(ped) then
        QBCore.Functions.Notify('This can only be done on foot..', 'error')
        return
    end

    local vehiclePlate = "QBR"..math.random(1, 100)
    TriggerServerEvent('ctn-vehiclerental:server:SetVehicleRented', vehiclePlate, true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('ctn-vehiclerental:server:SpawnRentedVehicle')
AddEventHandler('ctn-vehiclerental:server:SpawnRentedVehicle', function(vehiclePlate, vehicleData)
    local ped = GetPlayerPed(-1)
    local coords = {
...
ReturnVehicle = function()
    if RentedVehiclePlate ~= nil then
        Menu.addButton("Accept", "AcceptReturn", nil)
        Menu.addButton("Back", "RentalMenu", nil) 
        return
    end
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        ...
        TriggerServerEvent('ctn-vehiclerental:server:SetVehicleRented', RentedVehiclePlate, false, Config.RentalVehicles[RentedVehicleData])
...
RentalMenu = function()
    ClearMenu()
    Menu.addButton("Vehicle rent", "RentVehicleMenu", nil)
