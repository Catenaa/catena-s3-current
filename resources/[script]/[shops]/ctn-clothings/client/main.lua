...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ctn-clothesd:loadPlayerSkin")
...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        if Config.Stores[k].shopType == "clothing" then
            ...
            AddTextComponentString("Clothingstore")
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            ...
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - To adjust her')
                            end
                            if IsControlJustPressed(0, Keys["E"]) then
                                ...
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Her", selected = true},
                                    })
                                end
                            end
                        end
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    QBCore.Functions.TriggerCallback('ctn-clothings:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('ctn-clothings:client:openOutfitMenu')
AddEventHandler('ctn-clothings:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('ctn-clothings:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('ctn-clothings:client:loadOutfit', data)
end)
...
RegisterNetEvent('ctn-clothings:client:openMenu')
AddEventHandler('ctn-clothings:client:openMenu', function()
    customCamLocation = nil
    openMenu({
        {menu = "character", label = "Character", selected = true},
        {menu = "clothing", label = "Appearance", selected = false},
        {menu = "accessoires", label = "Accessories", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent('chatMessage', "SYSTEM", "error", "You cannot remove your ankle bracelet..")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('ctn-clothesd:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('ctn-clothings:client:reloadOutfits')
AddEventHandler('ctn-clothings:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('ctn-clothings:server:removeOutfit', data.outfitName, data.outfitId)
    TriggerEvent('chatMessage', "SYSTEM", "warning", "U removed "..data.outfitName.." !")
end)
...
function SaveSkin()
	...
	TriggerServerEvent("ctn-clothings:saveSkin", model, clothing)
end

RegisterNetEvent('ctn-clothesd:client:CreateFirstCharacter')
AddEventHandler('ctn-clothesd:client:CreateFirstCharacter', function()
...
RegisterNetEvent("ctn-clothesd:loadSkin")
AddEventHandler("ctn-clothesd:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('ctn-clothings:client:loadPlayerClothing', data, GetPlayerPed(-1))
    end)
end)

RegisterNetEvent('ctn-clothings:client:loadPlayerClothing')
AddEventHandler('ctn-clothings:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('ctn-clothings:client:loadOutfit')
AddEventHandler('ctn-clothings:client:loadOutfit', function(oData)
    local ped = GetPlayerPed(-1)
    ...
    if oData.outfitName ~= nil then
        TriggerEvent('chatMessage', "SYSTEM", "warning", "The outfit "..oData.outfitName.." is chosen! Press Confirm to confirm outfit.")
    end
end)
...
RegisterNetEvent("ctn-clothings:client:adjustfacewear")
AddEventHandler("ctn-clothings:client:adjustfacewear",function(type)
