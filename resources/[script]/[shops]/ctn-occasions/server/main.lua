...
QBCore.Functions.CreateCallback('ctn-occasions:server:getVehicles', function(source, cb)
...
QBCore.Functions.CreateCallback("ctn-garage:server:checkVehicleOwner", function(source, cb, plate)
    local src = source
...
QBCore.Functions.CreateCallback("ctn-occasions:server:getSellerInformation", function(source, cb, citizenid)
    local src = source
...
RegisterServerEvent('ctn-occasions:server:ReturnVehicle')
AddEventHandler('ctn-occasions:server:ReturnVehicle', function(vehicleData)
    local src = source
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `occasion_vehicles` WHERE `plate` = '"..vehicleData['plate'].."' AND `occasionid` = '"..vehicleData["oid"].."'", function(result)
        if result[1] ~= nil then 
            if result[1].seller == Player.PlayerData.citizenid then
                ...
                TriggerClientEvent("ctn-occasions:client:ReturnOwnedVehicle", src, result[1])
                TriggerClientEvent('ctn-occasion:client:refreshVehicles', -1)
            else
...
RegisterServerEvent('ctn-occasions:server:sellVehicle')
AddEventHandler('ctn-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerEvent("ctn-logs:server:sendLog", Player.PlayerData.citizenid, "vehiclesold", {model=vehicleData.model, vehiclePrice=vehiclePrice})
    TriggerEvent("ctn-logs:server:CreateLog", "vehicleshop", "Vehicle for sale", "red", "**"..GetPlayerName(src) .. "** bought a " .. vehicleData.model .. " for "..vehiclePrice)

    TriggerClientEvent('ctn-occasion:client:refreshVehicles', -1)
end)

RegisterServerEvent('ctn-occasions:server:buyVehicle')
AddEventHandler('ctn-occasions:server:buyVehicle', function(vehicleData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `occasion_vehicles` WHERE `plate` = '"..vehicleData['plate'].."' AND `occasionid` = '"..vehicleData["oid"].."'", function(result)
        if result[1] ~= nil and next(result[1]) ~= nil then
            if Player.PlayerData.money.cash >= result[1].price then
                ...
                TriggerEvent("ctn-logs:server:sendLog", Player.PlayerData.citizenid, "vehiclebought", {model = result[1].model, from = SellerCitizenId, moneyType = "cash", vehiclePrice = result[1].price, plate = result[1].plate})
                TriggerEvent("ctn-logs:server:CreateLog", "vehicleshop", "Occasion sold", "green", "**"..GetPlayerName(src) .. "** sold an occasion for "..result[1].price .. " (" .. result[1].plate .. ") to **"..SellerCitizenId.."**")
                TriggerClientEvent('ctn-occasion:client:refreshVehicles', -1)
                ...
                TriggerEvent('ctn-phone:server:sendNewMailToOffline', SellerCitizenId, {
                    sender = "Mosleys Occasions",
                    subject = "U heeft een voertuig verkocht!",
                    message = ""..QBCore.Shared.Vehicles[result[1].model].name.." is sold for €"..result[1].price..",-!"
                })
            elseif Player.PlayerData.money.bank >= result[1].price then
                ...
                TriggerEvent("ctn-logs:server:sendLog", Player.PlayerData.citizenid, "vehiclebought", {model = result[1].model, from = SellerCitizenId, moneyType = "cash", vehiclePrice = result[1].price, plate = result[1].plate})
                TriggerEvent("ctn-logs:server:CreateLog", "vehicleshop", "Bought second-hand", "green", "**"..GetPlayerName(src) .. "** bought an occasional for "..result[1].price .. " (" .. result[1].plate .. ") from **"..SellerCitizenId.."**")
                TriggerClientEvent('ctn-occasion:client:refreshVehicles', -1)
                ...
                TriggerEvent('ctn-phone:server:sendNewMailToOffline', SellerCitizenId, {
                    ...
                    message = "Je "..QBCore.Shared.Vehicles[result[1].model].name.." is sold for €"..result[1].price..",-!"
                })
            else
                TriggerClientEvent('QBCore:Notify', src, 'You don\'t have enough money...', 'error', 3500)
            end
        end
    end)
end)
