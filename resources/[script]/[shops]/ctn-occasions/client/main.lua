            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        QBCore.Functions.TriggerCallback('ctn-occasions:server:getVehicles', function(vehicles)
                        ...
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    QBCore.Functions.TriggerCallback('ctn-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                                ...
                                if IsDisabledControlJustPressed(0, Keys["7"]) then
                                    isConfirming = false
                                    currentVehicle = i
                                    TriggerServerEvent("ctn-occasions:server:ReturnVehicle", Config.OccasionSlots[i])
                                end
                ...
                if sellDist <= 13.0 and IsPedInAnyVehicle(ped) then 
                    ...
                    if sellDist <= 3.5 and IsPedInAnyVehicle(ped) then
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            QBCore.Functions.TriggerCallback('ctn-garage:server:checkVehicleOwner', function(owned)
                                if owned then
...
RegisterNUICallback('buyVehicle', function()
    ...
    TriggerServerEvent('ctn-occasions:server:buyVehicle', vehData)
end)

DoScreenFadeIn(250)

RegisterNetEvent('ctn-occasions:client:BuyFinished')
AddEventHandler('ctn-occasions:client:BuyFinished', function(model, plate, mods)
    ...
    QBCore.Functions.SpawnVehicle(model, function(veh)
        ...
        exports['ctn-hud']:SetFuel(veh, 100)
...
RegisterNetEvent('ctn-occasions:client:ReturnOwnedVehicle')
AddEventHandler('ctn-occasions:client:ReturnOwnedVehicle', function(vehdata)
    ...
    QBCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
        exports['ctn-hud']:SetFuel(veh, 100)
        QBCore.Functions.Notify("Your vehicle is returned..")
...
RegisterNUICallback('sellVehicle', function(data)
    ...
    TriggerServerEvent('ctn-occasions:server:sellVehicle', data.price, vehicleData)
...
function sellVehicleWait(price)
    ...
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    QBCore.Functions.Notify('Your car is for sale, €'..price..',-', 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

RegisterNetEvent('ctn-occasion:client:refreshVehicles')
AddEventHandler('ctn-occasion:client:refreshVehicles', function()
    if inRange then
        QBCore.Functions.TriggerCallback('ctn-occasions:server:getVehicles', function(vehicles)
            occasionVehicles = vehicles
            despawnOccasionsVehicles()
