...
$(document).ready(function(){
    console.log("ctn-customs: Succesfully loaded!");
...
function ChangeOption(type) {
    ...
    if (MenuData.CurrentModId !== "shoppingcart") {
        $.post('http://ctn-customs/OnIndexChange', JSON.stringify({
...
function SubmitOption() {
    ...
    if (OptionData.id !== "repairvehicle") {
        if (OptionData.id !== "close-menu" && OptionData.id !== "shoppingcart" && OptionData.id !== "main-menu" && OptionData.id !== "buy-upgrades") {
            ...
            $.post('http://ctn-customs/GetCurrentMod', JSON.stringify({
                data: OptionData
                ...
                $.post('http://ctn-customs/GetCartItem', JSON.stringify({
                    data: checkdata,
                }), function(marketitem){
                    if (OptionData.id !== null && OptionData.id !== undefined) {
                    ...
                    if (OptionData.buttons !== null && OptionData.buttons !== undefined) {
                        $.post('http://ctn-customs/SelectSound');
                                ...
                                if (mod.price !== null && mod.price !== undefined) {
                                    ...
                                    if (MenuData.CurrentModId == "respray" || MenuData.CurrentModId == "wheels" || MenuData.CurrentModId == "wheelaccessories" || installedmod > -1) {
                                        if (installedmod > -1 && i == installedmod + 1) {
                                            ...
                                            $.post('http://ctn-customs/ToggleCartedItem', JSON.stringify({toggle: true}));
                                        } else if (mod.price == 0) {
                                            mod.bought = false;
                            ...
                            if (!OptionData.cart || OptionData.cart === undefined){
                                ...
                                if (CurrentEquiped.length > 0) {
                                    ...
                                    if (!$("[data-menuindex='" + MenuData.SelectedIndex + "']").hasClass('cart-option')){
                                        ...
                                        $.post('http://ctn-customs/RemoveCartItem', JSON.stringify({
                                        ...
                                        $.post('http://ctn-customs/AddItemToCart', JSON.stringify({
                                            ItemData: OptionData,
                                        }));
                                    } else {
                                        $(".cart-option").find('.menu-option-price').html(pricelabel);
            ...
            $.post('http://ctn-customs/SelectSound');
            $.post('http://ctn-customs/PurchaseUpgrades');
            ...
            $.post('http://ctn-customs/QuitSound');
            $(".menu-container").animate({
                left: -45+"vh",
            }, 300, function(){
                $(".qbus-customs").css({"display":"none"});
            ...
            MenuData.SubMenu = [];
            MenuData.CurrentMenu = 0;
            $.post('http://ctn-customs/CloseMenu');
        }
    } else {
        $.post('http://ctn-customs/CanRepairVehicle', JSON.stringify({
...
function Back() {
    $.post('http://ctn-customs/BackSound');
    if (MenuData.CurrentMenu !== "shoppingcart") {
        if (MenuData.SubMenu[0] !== null && MenuData.SubMenu[0] !== undefined) {
            if (MenuData.CurrentMenu - 1 !== 0) {
                ...
                $.post('http://ctn-customs/CheckIfCartedItem');
                $(".menu-options").html("");
                $("#menu-name").html("Welcome to Benny's Motorsports");
                $("#menu-help").html("Choose a menu");
                MenuData.TotalOptions = 0;
            ...
            if (MenuData.CurrentMenu - 1 >= -1) {
                $.post('http://ctn-customs/GetShoppingCart', JSON.stringify({}), function(ShoppingCart){
                    $(".menu-options").html("");
                    var ModElement;
                    if (ShoppingCart.buttons.length > 0) {
                        ModElement = '<div class="menu-option" data-menuindex="1"><div class="menu-option-name">ShoppingCart <div class="menu-option-price"><i class="fas fa-shopping-cart equiped-check"></i> ' + ShoppingCart.buttons.length + '</div></div></div>'+
                        '<div class="menu-option" data-menuindex="2"><div class="menu-option-name">MainMenu</div></div>' +
                        ...
                        ModElement = '<div class="menu-option" data-menuindex="1"><div class="menu-option-name">MainMenu</div></div>'+
                        '<div class="menu-option" data-menuindex="2"><div class="menu-option-name">CloseMenu</div></div>';
                        $(".menu-options").append(ModElement);
