...
function OpenCustoms(shop, k)
    ...
    if k == 7 then
        QBCore.Functions.TriggerCallback('ctn-vehicletuning:server:IsMechanicAvailable', function(Mechanic)
            if Mechanic < 2 then
            ...
            TriggerServerEvent('ctn-customs:server:UpdateBusyState', k, true)
            CurrentShop = k
        end)
    else
        SetNuiFocus(true, false)
...
RegisterNUICallback('CanRepairVehicle', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('ctn-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
	        SetVehicleFixed(veh)
                        ...
                        if IsPedInAnyVehicle(ped) then
                            ...
                            if seat == ped then
                                if not IsVehicleBlacklisted(GetEntityModel(GetVehiclePedIsIn(ped))) then
                                    ...
                                    if distance < 3 then
                                        DrawText3Ds(loc["coords"].x, loc["coords"].y, loc["coords"].z, 'Press ~b~ENTER~w~ to enter Benny\'s.')
...
RegisterNUICallback('CloseMenu', function()
    ...
    TriggerServerEvent('ctn-customs:server:UpdateBusyState', CurrentShop, false)
...
RegisterNUICallback('print', function(data, cb)
    -- TriggerServerEvent('ctn-customs:print', data.print)
end)
...
RegisterNetEvent('ctn-customs:client:UpdateBusyState')
AddEventHandler('ctn-customs:client:UpdateBusyState', function(k, bool)
    ...
    QBCore.Functions.TriggerCallback('ctn-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
            ...
            if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
                ...
                TriggerServerEvent("ctn-log:server:sendLog", PlayerData.citizenid, "buy", {upgrades = String, citizenid = PlayerData.citizenid})
	            TriggerServerEvent("ctn-log:server:CreateLog", "bennys", "buy", "green", "**" .. PlayerData.name .. "** (citizenid: *" .. PlayerData.citizenid .. "* | id: *(" .. PlayerData.source .. ")* bought for **€" .. TotalPrice .. "** purchased the following upgrades: \n" .. String)
            end
            ...
            TriggerServerEvent('ctn-customs:server:SaveVehicleProps', CurrentVehicleData)
        else
            QBCore.Functions.Notify('You don\'t have enough money..', 'error')
        end
    end, TotalPrice)
