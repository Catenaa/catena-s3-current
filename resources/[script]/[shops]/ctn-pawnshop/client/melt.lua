...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
    QBCore.Functions.TriggerCallback('ctn-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("ctn-pawnshop:server:getGoldBars")
                        end
                    else
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then 
                            ...
                            ScrapAnim(1000)
                            QBCore.Functions.Progressbar("drop_golden_stuff", "Grabbing items", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("ctn-pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
...
function HasPlayerGold()
	local retval = false
	QBCore.Functions.TriggerCallback('ctn-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('ctn-pawnshop:client:startMelting')
AddEventHandler('ctn-pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('ctn-pawnshop:client:SetTakeState')
AddEventHandler('ctn-pawnshop:client:SetTakeState', function(state)
