...
RegisterNetEvent('ctn-shops:client:UpdateShop')
AddEventHandler('ctn-shops:client:UpdateShop', function(shop, itemData, amount)
    TriggerServerEvent('ctn-shops:server:UpdateShopItems', shop, itemData, amount)
    TriggerEvent("debug", 'Shops: Updated', 'success')
end)

RegisterNetEvent('ctn-shops:client:SetShopItems')
AddEventHandler('ctn-shops:client:SetShopItems', function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent('ctn-shops:client:RestockShopItems')
AddEventHandler('ctn-shops:client:RestockShopItems', function(shop, amount)
...
Citizen.CreateThread(function()
    for store,_ in pairs(Config.Locations) do
        ...
        elseif Config.Locations[store]["products"] == Config.Products["gsgasoline"] then
            ...
            SetBlipScale(StoreBlip, 0.4)
            SetBlipColour(StoreBlip, 2)     
        elseif Config.Locations[store]["products"] == Config.Products["diving"] then
            SetBlipSprite(StoreBlip, 371)
