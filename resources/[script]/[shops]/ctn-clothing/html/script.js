...
QBClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('http://ctn-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('http://ctn-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('http://ctn-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('http://ctn-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('http://ctn-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('http://ctn-clothing/rotateRight');
            break;
        case 65: // A
            $.post('http://ctn-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    QBClothing.Close();
    $.post('http://ctn-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    QBClothing.Close();
    $.post('http://ctn-clothing/resetOutfit');
});
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('http://ctn-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('http://ctn-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('http://ctn-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('http://ctn-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
