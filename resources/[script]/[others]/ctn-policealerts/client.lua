...
AddEventHandler('QBCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
    if JobInfo.name == "police" or JobInfo.name == "gendarme" then
...
RegisterNetEvent('ctn-policealerts:ToggleDuty')
AddEventHandler('ctn-policealerts:ToggleDuty', function(Duty)
    PlayerJob.onduty = Duty
end)

RegisterNetEvent('ctn-policealerts:client:AddPoliceAlert')
AddEventHandler('ctn-policealerts:client:AddPoliceAlert', function(data, forBoth)
    if forBoth then
        if (PlayerJob.name == "gendarme" or PlayerJob.name == "police" or PlayerJob.name == "doctor" or PlayerJob.name == "ambulance") and PlayerJob.onduty then
            if data.alertTitle == "Assistance collega" or data.alertTitle == "Assistance Ambulance" or data.alertTitle == "Doctor assistance" then
    ...
    elseif forBoth == 'gendarme' then
        if (PlayerJob.name == "gendarme" and PlayerJob.onduty) then
            if data.alertTitle == "Assistance collega" or data.alertTitle == "Assistance Ambulance" or data.alertTitle == "Doctor assistance" then
                TriggerServerEvent("InteractSound_SV:PlayOnSource", "emergency", 0.7)
            else
                PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
        ...
        if ((PlayerJob.name == "police" or PlayerJob.name == "gendarme") and PlayerJob.onduty) then
            if data.alertTitle == "Assistance collega" or data.alertTitle == "Assistance Ambulance" or data.alertTitle == "Doctor assistance" then
                TriggerServerEvent("InteractSound_SV:PlayOnSource", "emergency", 0.7)
            else
                PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
