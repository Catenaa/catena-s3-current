...
QBCore.Functions.CreateCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if Player.PlayerData.money.cash >= price then
		...
		print("You have bought the " .. tattooName .. " tattoo for $" .. price)
		cb(true)


		TriggerClientEvent('Apply:Tattoo', source)
	else
		TriggerClientEvent('QBCore:Notify', source, "Ma3andkch flous!", "error")
    end
end)
...
AddEventHandler('Select:Tattoos', function() 
	...
	exports['ghmattimysql']:execute( 'SELECT tattoos FROM players WHERE citizenid = @citizenid', {
		...
		if result[1].tattoos ~= nil then
		    if result[1].tattoos then
		     	TriggerClientEvent('Apply:Tattoo', src)
		    else
...
AddEventHandler('remover:all', function() 
	...
	local Player = QBCore.Functions.GetPlayer(src)
	if Player then
		exports['ghmattimysql']:execute( 'UPDATE character_current SET tattoos = @tattoos WHERE citizenid = @citizenid', {
			['@tattoos'] = 0,
			['@citizenid'] = Player.PlayerData.citizenid
		})
	end
