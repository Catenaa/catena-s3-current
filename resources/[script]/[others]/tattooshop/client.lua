...
AddEventHandler('Apply:Tattoo', function() 
...
----------------------------------------------------------------------------------
---- This shit doesnt make sense ill try and add tattoos to peds again later------
----------------------------------------------------------------------------------
...
-------------------This thread will run through the list of available Tattoos and send them to the proper regions rather than being grouped up in dlcs *YUCK*----------------
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(300000)
...
------------------If you are running the base Qbus you should still have the ability for skinchanger i myself do not use it so i make my players do it manually with the altered dpclothing --------------
function GetNaked()
...
---------- if you dont purchase the tattoo this function removes it from being previewed on your ped------------
function ResetSkin()
	ClearPedDecorations(PlayerPedId())
	for k, v in pairs(currentTattoos) do
		if v.Count ~= nil then
...
-----------------------------loads the information for the tattoos --------------------------------------
...
------------------Opens the tattoo shops -----------------------
function OpenTattooShop()
	JayMenu.OpenMenu("tattoo")
	FreezeEntityPosition(PlayerPedId(), true)
	ReqTexts("TAT_MNU", 9)
end
---------------------function for closing the tattoo shop menu-----------
...
-------------function for allowing you to cycle through the available tattoo sections --------------
function ButtonPress()
	PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
end
---------------function to open the menu when in range and button is pressed ----------------
function IsMenuOpen()
	return (JayMenu.IsMenuOpened('tattoo') or string.find(tostring(JayMenu.CurrentMenu() or ""), "ZONE_"))	
end
------------------Function to purchase the tattoo-----------------------
...
---------Dev command when testing the tattoos so you do not have to cycle to remove all pre existing -------------
--[[ RegisterCommand('resetink',function(source)
	TriggerServerEvent('Select:Tattoos')
...
------------------function Removes the tattoos when you deselect them from the menu------------------
...
------------sets proper camera distance and tattoo scaling -----------------
...
-----------Thank you Jay for this menu you fuckin rock dude!-------------------
--------also sitizen thread making surew the menu is populated-------------
Citizen.CreateThread(function()
...
----------------shows all the names of the tattoos and buttons
function ButtonMessage(text)
    BeginTextCommandScaleformString("STRING")
    AddTextComponentScaleform(text)
    EndTextCommandScaleformString()
end
------------when shift is pressed moves on to the next cam angle
...
----------draws text to let you know where tf the shop is----------
...
---------------all the different places tattooshops are-------------
Config.coordinates = {
...
------------------dont recommend activating this its mildly broken
--[[Citizen.CreateThread(function()
