                ...
                for k, race in pairs(Races) do
                    if Races[k] ~= nil then
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[k].startx, Races[k].starty, Races[k].startz, true) < 15.0 and not Races[k].started then
                            DrawText3Ds(Races[k].startx, Races[k].starty, Races[k].startz, "[~g~H~w~] To participate in the race (~g~€"..Races[k].amount..",-~w~)")
                            if IsControlJustReleased(0, Keys["H"]) then
                                TriggerServerEvent("ctn-streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 250.0 and Races[RaceId].started then
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 15.0 then
                        TriggerServerEvent("ctn-streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('ctn-streetraces:StartRace')
AddEventHandler('ctn-streetraces:StartRace', function(race)
...
RegisterNetEvent('ctn-streetraces:RaceDone')
AddEventHandler('ctn-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        QBCore.Functions.Notify("Race ended! The winner is "..winner.. "!")
    end
end)

RegisterNetEvent('ctn-streetraces:StopRace')
AddEventHandler('ctn-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('ctn-streetraces:CreateRace')
AddEventHandler('ctn-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(GetPlayerPed(-1), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            TriggerServerEvent("ctn-streetraces:NewRace", race)
...
RegisterNetEvent('ctn-streetraces:SetRace')
AddEventHandler('ctn-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('ctn-streetraces:SetRaceId')
AddEventHandler('ctn-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
