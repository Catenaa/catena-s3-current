...
RegisterServerEvent("ctn-bossmenu:server:withdrawMoney")
AddEventHandler("ctn-bossmenu:server:withdrawMoney", function(amount)
    local src = source
    ...
    TriggerClientEvent('ctn-bossmenu:client:refreshSociety', -1, job, Accounts[job])
    SaveResourceFile(GetCurrentResourceName(), "./database.json", json.encode(Accounts), -1)
    TriggerEvent('ctn-logs:server:createLog', 'bossmenu', 'Withdraw Money', "Successfully withdrawn $" .. amount .. ' (' .. job .. ')', src)
end)

RegisterServerEvent("ctn-bossmenu:server:depositMoney")
AddEventHandler("ctn-bossmenu:server:depositMoney", function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local job = xPlayer.PlayerData.job.name
    ...
    TriggerClientEvent('ctn-bossmenu:client:refreshSociety', -1, job, Accounts[job])
    SaveResourceFile(GetCurrentResourceName(), "./database.json", json.encode(Accounts), -1)
    TriggerEvent('ctn-logs:server:createLog', 'bossmenu', 'Deposit Money', "Successfully deposited $" .. amount .. ' (' .. job .. ')', src)
end)

RegisterServerEvent("ctn-bossmenu:server:addAccountMoney")
AddEventHandler("ctn-bossmenu:server:addAccountMoney", function(account, amount)
    if not Accounts[account] then
        Accounts[account] = 0
    end
    
    Accounts[account] = Accounts[account] + amount
    TriggerClientEvent('ctn-bossmenu:client:refreshSociety', -1, account, Accounts[account])
    SaveResourceFile(GetCurrentResourceName(), "./database.json", json.encode(Accounts), -1)
end)

RegisterServerEvent("ctn-bossmenu:server:removeAccountMoney")
AddEventHandler("ctn-bossmenu:server:removeAccountMoney", function(account, amount)
    if not Accounts[account] then
        Accounts[account] = 0
    end
...
RegisterServerEvent("ctn-bossmenu:server:openMenu")
AddEventHandler("ctn-bossmenu:server:openMenu", function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if job.isboss == true then
        ...
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `job` LIKE '%".. job.name .."%'", function(players)
            ...
            TriggerClientEvent('ctn-bossmenu:client:openMenu', src, employees, QBCore.Shared.Jobs[job.name])
            TriggerClientEvent('ctn-bossmenu:client:refreshSociety', -1, job.name, Accounts[job.name])
        end)
    else
...
RegisterServerEvent('ctn-bossmenu:server:fireEmployee')
AddEventHandler('ctn-bossmenu:server:fireEmployee', function(data)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if xEmployee then
        if xEmployee.Functions.SetJob("unemployed", '0') then
            TriggerEvent('ctn-logs:server:createLog', 'bossmenu', 'Job Fire', "Successfully fired " .. GetPlayerName(xEmployee.PlayerData.source) .. ' (' .. xPlayer.PlayerData.job.name .. ')', src)
            ...
            QBCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `job` LIKE '%".. xPlayer.PlayerData.job.name .."%'", function(players)
                if players[1] ~= nil then
                    ...
                    TriggerClientEvent('ctn-bossmenu:client:refreshPage', src, 'employee', employees)
                end
            end)
        else
        ...
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '" .. data.source .. "' LIMIT 1", function(player)
            if player[1] ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', src, "Fired successfully!", "success")
                TriggerEvent('ctn-logs:server:createLog', 'bossmenu', 'Fire', "Successfully fired " .. data.source .. ' (' .. xPlayer.PlayerData.job.name .. ')', src)
...
RegisterServerEvent('ctn-bossmenu:server:giveJob')
AddEventHandler('ctn-bossmenu:server:giveJob', function(data)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if xPlayer.PlayerData.job.isboss == true then
        if xTarget and xTarget.Functions.SetJob(xPlayer.PlayerData.job.name, '0') then
            ...
            TriggerEvent('ctn-logs:server:createLog', 'bossmenu', 'Recruit', "Successfully recruited " .. (xTarget.PlayerData.charinfo.firstname .. ' ' .. xTarget.PlayerData.charinfo.lastname) .. ' (' .. job .. ')', src)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "You are not the boss, how did you reach here bitch?!", "error")
    end
...
RegisterServerEvent('ctn-bossmenu:server:updateGrade')
AddEventHandler('ctn-bossmenu:server:updateGrade', function(data)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local xEmployee = QBCore.Functions.GetPlayerByCitizenId(data.source)
...
RegisterServerEvent('ctn-bossmenu:server:updateNearbys')
AddEventHandler('ctn-bossmenu:server:updateNearbys', function(data)
    local src = source
    local players = {}
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ctn-bossmenu:client:refreshPage', src, 'recruits', players)
end)
