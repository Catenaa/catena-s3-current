...
RegisterNetEvent('ctn-drugs:client:cornerselling')
AddEventHandler('ctn-drugs:client:cornerselling', function(data)
    QBCore.Functions.TriggerCallback('ctn-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            ...
            QBCore.Functions.Notify('Ma3andch hchich alhmar', 'error')
        end
    end)
end)
...
RegisterNetEvent('ctn-drugs:client:refreshAvailableDrugs')
AddEventHandler('ctn-drugs:client:refreshAvailableDrugs', function(items)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('ctn-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                QBCore.Functions.Notify('Rah sr9ek, khda lik '..bagAmount..' dial '..availableDrugs[drugType].label..' tba3 9ahba mo', 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' b taman '..randomPrice..' DH? / ~g~G~w~ Decline offer')
                
                    if IsControlJustPressed(0, Keys["E"]) then
                        if availableDrugs[drugType].amount < bagAmount then
                            QBCore.Functions.Notify('M3andkch hchich kafi. sir jib lhchich', 'error')
                        end
                        TriggerServerEvent('ctn-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
