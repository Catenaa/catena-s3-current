...
QBCore.Functions.CreateCallback('ctn-weed:server:getBuildingPlants', function(source, cb, building)
...
QBCore.Commands.Add("placeplant", "", {}, false, function(source, args)
    ...
    if QBWeed.Plants[type] ~= nil then
        TriggerClientEvent('ctn-weed:client:placePlant', src, type)
    end
end)

RegisterServerEvent('ctn-weed:server:placePlant')
AddEventHandler('ctn-weed:server:placePlant', function(currentHouse, coords, sort)
    ...
    TriggerClientEvent('ctn-weed:client:refreshHousePlants', -1, currentHouse)
end)

RegisterServerEvent('ctn-weed:server:removeDeathPlant')
AddEventHandler('ctn-weed:server:removeDeathPlant', function(building, plantId)
    QBCore.Functions.ExecuteSql(true, "DELETE FROM `house_plants` WHERE plantid = '"..plantId.."' AND building = '"..building.."'")
    TriggerClientEvent('ctn-weed:client:refreshHousePlants', -1, building)
end)
...
Citizen.CreateThread(function()
    while true do
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM `house_plants`", function(housePlants)
            ...
            TriggerClientEvent('ctn-weed:client:refreshPlantStats', -1)
        end)
...
QBCore.Functions.CreateUseableItem("weed_white-widow_seed", function(source, item)
    ...
    TriggerClientEvent('ctn-weed:client:placePlant', source, 'white-widow', item)
end)

QBCore.Functions.CreateUseableItem("weed_skunk_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('ctn-weed:client:placePlant', source, 'skunk', item)
end)

QBCore.Functions.CreateUseableItem("weed_purple-haze_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('ctn-weed:client:placePlant', source, 'purple-haze', item)
end)

QBCore.Functions.CreateUseableItem("weed_og-kush_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('ctn-weed:client:placePlant', source, 'og-kush', item)
end)

QBCore.Functions.CreateUseableItem("weed_amnesia_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('ctn-weed:client:placePlant', source, 'amnesia', item)
end)

QBCore.Functions.CreateUseableItem("weed_ak47_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('ctn-weed:client:placePlant', source, 'ak47', item)
end)

QBCore.Functions.CreateUseableItem("weed_nutrition", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('ctn-weed:client:foodPlant', source, item)
end)

RegisterServerEvent('ctn-weed:server:removeSeed')
AddEventHandler('ctn-weed:server:removeSeed', function(itemslot, seed)
    local Player = QBCore.Functions.GetPlayer(source)
    Player.Functions.RemoveItem(seed, 1, itemslot)
end)

RegisterServerEvent('ctn-weed:server:harvestPlant')
AddEventHandler('ctn-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if weedBag ~= nil then
            ...
            if house ~= nil then 
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `house_plants` WHERE plantid = '"..plantId.."' AND building = '"..house.."'", function(result)
                    if result[1] ~= nil then
                        ...
                        TriggerClientEvent('ctn-weed:client:refreshHousePlants', -1, house)
                    else
        ...
        TriggerClientEvent('QBCore:Notify', src, 'You do not have resealable bags', 'error', 3500)
    end
end)

RegisterServerEvent('ctn-weed:server:foodPlant')
AddEventHandler('ctn-weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
