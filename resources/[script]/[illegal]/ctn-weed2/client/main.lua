...
RegisterNetEvent('ctn-weed:client:getHousePlants')
AddEventHandler('ctn-weed:client:getHousePlants', function(house)    
    QBCore.Functions.TriggerCallback('ctn-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                ...
                for k, v in pairs(housePlants[currentHouse]) do
                    ...
                    if plyDistance < 0.8 then
                            ...
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, QBWeed.Keys["E"]) then
                                    QBCore.Functions.Progressbar("remove_weed_plant", "Harvesting Plant", 8000, false, true, {
                                        ...
                                        TriggerServerEvent('ctn-weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                                    end, function() -- Cancel
                                        ClearPedTasks(ped)
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, QBWeed.Keys["E"]) then
                                QBCore.Functions.Progressbar("remove_weed_plant", "Removing Plant", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
                                    ...
                                    ClearPedTasks(ped)
                                    TriggerServerEvent('ctn-weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
                                    QBCore.Functions.Notify("Process canceled..", "error")
                                end)
...
RegisterNetEvent('ctn-weed:client:leaveHouse')
AddEventHandler('ctn-weed:client:leaveHouse', function()
    despawnHousePlants()
...
RegisterNetEvent('ctn-weed:client:refreshHousePlants')
AddEventHandler('ctn-weed:client:refreshHousePlants', function(house)
...
RegisterNetEvent('ctn-weed:client:refreshPlantStats')
AddEventHandler('ctn-weed:client:refreshPlantStats', function()
    if insideHouse then
        despawnHousePlants()
        SetTimeout(500, function()
            QBCore.Functions.TriggerCallback('ctn-weed:server:getBuildingPlants', function(plants)
...
RegisterNetEvent('ctn-weed:client:placePlant')
AddEventHandler('ctn-weed:client:placePlant', function(type, item)
    local ped = GetPlayerPed(-1)
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            QBCore.Functions.Progressbar("plant_weed_plant", "Planting..", 8000, false, true, {
                ...
                TriggerServerEvent('ctn-weed:server:placePlant', currentHouse, json.encode(plantData["plantCoords"]), type)
                TriggerServerEvent('ctn-weed:server:removeSeed', item.slot, type)
            ...
            QBCore.Functions.Notify('Not a safe place..', 'error', 3500)
        end
    else
        QBCore.Functions.Notify('It\'s not safe here..', 'error', 3500)
    end
end)

RegisterNetEvent('ctn-weed:client:foodPlant')
AddEventHandler('ctn-weed:client:foodPlant', function(item)
    local plantData = {}
    if currentHouse ~= nil then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    ...
                    QBCore.Functions.Progressbar("plant_weed_plant", "Feeding Plant", math.random(4000, 8000), false, true, {
                        ...
                        TriggerServerEvent('ctn-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                ...
                QBCore.Functions.Notify("Not the place to plant..", "error")
            end
        else
            QBCore.Functions.Notify("Not the place to plant..", "error")
        end
