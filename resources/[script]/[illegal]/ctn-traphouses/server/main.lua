...
RegisterServerEvent('ctn-traphouses:server:TakeoverHouse')
AddEventHandler('ctn-traphouses:server:TakeoverHouse', function(Traphouse)
    local src = source
    ...
    if not HasCitizenIdHasKey(CitizenId, Traphouse) then
        if Player.Functions.RemoveMoney('cash', Config.TakeoverPrice) then
            TriggerClientEvent('ctn-traphouses:client:TakeoverHouse', src, Traphouse)
        else
...
RegisterServerEvent('ctn-traphouses:server:AddHouseKeyHolder')
AddEventHandler('ctn-traphouses:server:AddHouseKeyHolder', function(CitizenId, TraphouseId, IsOwner)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            TriggerClientEvent('ctn-traphouses:client:SyncData', -1, TraphouseId, Config.TrapHouses[TraphouseId])
        else
...
QBCore.Commands.Add("entertraphouse", "Enter traphouse", {}, false, function(source, args)
    ...
    TriggerClientEvent('ctn-traphouses:client:EnterTraphouse', src)
end)

QBCore.Commands.Add("givingstairskeys", "Give keys from the traphouse", {{name = "id", help = "Player id"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
            ...
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        TriggerClientEvent('ctn-traphouses:client:SyncData', -1, Traphouse, Config.TrapHouses[Traphouse])
                    else
                ...
                TriggerClientEvent('QBCore:Notify', src, 'This person already has the keys..', 'error')
            end
        else
...
RegisterServerEvent('ctn-traphouses:server:TakeMoney')
AddEventHandler('ctn-traphouses:server:TakeMoney', function(TraphouseId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'There is no money in the cash', 'error')
    end
end)
...
RegisterServerEvent('ctn-traphouses:server:RobNpc')
AddEventHandler('ctn-traphouses:server:RobNpc', function(Traphouse)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-traphouses:server:GetTraphousesData', function(source, cb)
