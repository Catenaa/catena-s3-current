...
Citizen.CreateThread(function()
    ...
    if QBCore.Functions.GetPlayerData() ~= nil then
        ...
        QBCore.Functions.TriggerCallback('ctn-traphouses:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('ctn-traphouses:client:EnterTraphouse')
AddEventHandler('ctn-traphouses:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        QBCore.Functions.Notify('The code is incorrect..', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('ctn-traphouses:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z, '~b~E~w~ - Take over the staircase (~g~€5000~w~)')
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent('ctn-traphouses:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z - 0.2, '~b~/givingstairskeys~w~ [id] - To give keys')
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent("ctn-traphouses:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
function LeaveTraphouse(data)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    exports['ctn-interior']:DespawnInterior(TraphouseObj, function()
...
RegisterNetEvent('ctn-traphouses:client:TakeoverHouse')
AddEventHandler('ctn-traphouses:client:TakeoverHouse', function(TraphouseId)
    local ped = GetPlayerPed(-1)

    QBCore.Functions.Progressbar("takeover_traphouse", "Taking over the staircase", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('ctn-traphouses:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            QBCore.Functions.Notify(CitizenId..' has been added to the stair house!')
        else
            QBCore.Functions.Notify(CitizenId..' this person already has keys!')
        end
    else
        QBCore.Functions.Notify('You can give max 6 other access to the stair house!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('ctn-traphouses:client:SyncData')
AddEventHandler('ctn-traphouses:client:SyncData', function(k, data)
