...
RegisterNetEvent('CTN-bankrobbery:UseBankcardA')
AddEventHandler('CTN-bankrobbery:UseBankcardA', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local dist = GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["coords"]["x"], Config.BigBanks["paleto"]["coords"]["y"],Config.BigBanks["paleto"]["coords"]["z"])
                ...
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        ...
                        QBCore.Functions.Progressbar("security_pass", "Adjust to validate", 7500, false, true, {
                            ...
                            TriggerServerEvent('CTN-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("QBCore:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('CTN-doorlock:server:updateState', 78, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("CTN-bankrobbery:server:callCops", "paleto", 0)
