...
RegisterNetEvent('CTN-bankrobbery:client:SetStationStatus')
AddEventHandler('CTN-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(GetPlayerPed(-1))
    while time > 0 do 
        QBCore.Functions.Notify("Burning over " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    ...
    if currentStation ~= 0 then
        ...
        TriggerServerEvent("CTN-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        QBCore.Functions.Notify("The door is burned open", "success")
        TriggerServerEvent('CTN-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
