...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
    ...
    QBCore.Functions.TriggerCallback('CTN-bankrobbery:server:GetConfig', function(config)
        Config = config
    end)
    onDuty = true
    ResetBankDoors()
...
AddEventHandler('electronickit:UseElectronickit', function()
    ...
    if closestBank ~= nil then
        QBCore.Functions.TriggerCallback('CTN-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if closestBank ~= nil then
                    local dist = GetDistanceBetweenCoords(pos, Config.SmallBanks[closestBank]["coords"]["x"], Config.SmallBanks[closestBank]["coords"]["y"], Config.SmallBanks[closestBank]["coords"]["z"])
                    if dist < 1.5 then
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        QBCore.Functions.Progressbar("hack_gate", "Connecting Electronic Kit", 7500, false, true, {
                                            ...
                                            if not copsCalled then
                                                ...
                                                if Config.SmallBanks[closestBank]["alarm"] then
                                                    TriggerServerEvent("CTN-bankrobbery:server:callCops", "small", closestBank)
                                        ...
                                        QBCore.Functions.Notify("You miss an item", "error")
                                    end
                                end, "trojan_usb")
                            else
                                QBCore.Functions.Notify("It seems that the bank is already open", "error")
                            end
                        else
                            QBCore.Functions.Notify("Not enough police", "error")
                        end
...
RegisterNetEvent('CTN-bankrobbery:client:setBankState')
AddEventHandler('CTN-bankrobbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('CTN-bankrobbery:client:enableAllBankSecurity')
AddEventHandler('CTN-bankrobbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('CTN-bankrobbery:client:disableAllBankSecurity')
AddEventHandler('CTN-bankrobbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('CTN-bankrobbery:client:BankSecurity')
AddEventHandler('CTN-bankrobbery:client:BankSecurity', function(key, status)
...
function openLocker(bankId, lockerId)
    ...
    TriggerServerEvent('CTN-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                if lockerId ~= 9 then
                    ...
                    IsDrilling = true
                    QBCore.Functions.Progressbar("open_locker_drill", "Cracking Safe", 50000, false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        TriggerServerEvent('CTN-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                        TriggerServerEvent('CTN-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                        TriggerServerEvent('CTN-bankrobbery:server:recieveItem', 'paleto', lockerId)
                    ...
                    Citizen.CreateThread(function()
                        while IsDrilling do
                            TriggerServerEvent('CTN-hud:Server:GainStress', math.random(4, 8))
                            Citizen.Wait(10000)
                        end
                    end)
                else
                    QBCore.Functions.Progressbar("open_locker_drill", "Searching Card", 45000, false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
    ...
    elseif bankId == "pacific" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                ...
                QBCore.Functions.Progressbar("open_locker_drill", "Cracking Safe", 50000, false, true, {
                    ...
                    TriggerServerEvent('CTN-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('CTN-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('CTN-bankrobbery:server:recieveItem', 'pacific')
                    QBCore.Functions.Notify("Success", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(GetPlayerPed(-1), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Cracking Safe", 45000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('CTN-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('CTN-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('CTN-bankrobbery:server:recieveItem', 'small', lockerId)
...
RegisterNetEvent('CTN-bankrobbery:client:setLockerState')
AddEventHandler('CTN-bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('CTN-bankrobbery:client:ResetFleecaLockers')
AddEventHandler('CTN-bankrobbery:client:ResetFleecaLockers', function(BankId)
...
function OnHackDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('CTN-bankrobbery:server:setBankState', closestBank, true)
    else
		TriggerEvent('mhacking:hide')
	end
end
