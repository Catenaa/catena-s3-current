...
Citizen.CreateThread(function()
    ...
    local requiredItems = {
		[1] = {name = QBCore.Shared.Items["electronickit"]["name"], image = QBCore.Shared.Items["electronickit"]["image"]},
		[2] = {name = QBCore.Shared.Items["security_card_02"]["name"], image = QBCore.Shared.Items["security_card_02"]["image"]},
    }
    while true do
    ...
    if dist < 1.5 then
        QBCore.Functions.TriggerCallback('CTN-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                                    ...
                                    QBCore.Functions.Progressbar("hack_gate", "Connecting Electronic Kit", 7500, false, true, {
                                        ...
                                        if not copsCalled then
                                            ...
                                            if Config.BigBanks["pacific"]["alarm"] then
                                                TriggerServerEvent("CTN-bankrobbery:server:callCops", "pacific", 0)
                ...
                QBCore.Functions.Notify("The security lock is active, the door cannot be opened atm", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('CTN-bankrobbery:UseBankcardB')
AddEventHandler('CTN-bankrobbery:UseBankcardB', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
        ...
        QBCore.Functions.TriggerCallback('CTN-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        QBCore.Functions.Progressbar("security_pass", "Adjust to validate", 7500, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('CTN-doorlock:server:updateState', 67, false)
...
function OnHackPacificDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('CTN-bankrobbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
	end
end
