                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if math.random(100) <= 50 then
                                        TriggerServerEvent('ctn-hud:Server:GainStress', math.random(1, 3))
                                    end
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        QBCore.Functions.TriggerCallback('ctn-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if copsCalled == false then
                                        ...
                                        TriggerServerEvent("ctn-storerobbery:server:callCops", "safe", currentSafe)
                                        copsCalled = true
                                    end
                                else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if math.random(1, 10) < 5 then
                        TriggerServerEvent("ctn-storerobbery:server:callCops", "cashier", currentRegister)   
                    ...
                    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                        if result then
                            ...
                            QBCore.Functions.Notify("You are missing a screwdriver", "error")
                        end
                    end, "screwdriverset")
                end
...
function setupRegister()
    QBCore.Functions.TriggerCallback('ctn-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    QBCore.Functions.TriggerCallback('ctn-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('ctn-storerobbery:server:setRegisterStatus', currentRegister)
        ...
        QBCore.Functions.Progressbar("search_register", "Emptying Register", lockpickTime, false, true, {
            ...
            TriggerServerEvent('ctn-storerobbery:server:takeMoney', currentRegister, true)            
            currentRegister = 0
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
...
function LockpickDoorAnim(time)
    ...
    Citizen.CreateThread(function()
        while openingDoor do
            ...
            Citizen.Wait(2000)
            time = time - 2
            TriggerServerEvent('ctn-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("ctn-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("ctn-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('fail', function()
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        QBCore.Functions.Notify("The lockpick bent out of shape.")
    end
    lockpick(false)
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    QBCore.Functions.TriggerCallback('ctn-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('ctn-storerobbery:client:setRegisterStatus')
AddEventHandler('ctn-storerobbery:client:setRegisterStatus', function(register, bool)
    Config.Registers[register].robbed = bool
end)

RegisterNetEvent('ctn-storerobbery:client:setSafeStatus')
AddEventHandler('ctn-storerobbery:client:setSafeStatus', function(safe, bool)
