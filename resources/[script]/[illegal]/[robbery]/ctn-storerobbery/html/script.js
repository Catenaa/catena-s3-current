...
Padlock.Close = function() {
    ...
    $.post('http://ctn-storerobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('http://ctn-storerobbery/PadLockClose');
    if (data.error != null) {
        $.post('http://ctn-storerobbery/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("http://ctn-storerobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('http://ctn-storerobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('http://ctn-storerobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            if (CurrentType == "keypad") {
                ...
                $.post('http://ctn-storerobbery/exit');
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post('http://ctn-storerobbery/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('http://ctn-storerobbery/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
