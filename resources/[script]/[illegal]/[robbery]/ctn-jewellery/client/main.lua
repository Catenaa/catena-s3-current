...
Citizen.CreateThread(function()
    ...
    QBCore.Functions.TriggerCallback('ctn-jewellery:gettimeoutstatus', function(rs)
        isInTimeout = rs
    end)
    isLoggedIn = true
end)
                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        ...
                                        TriggerServerEvent('ctn-jewellery:server:startglobaltimeout')
                                        QBCore.Functions.TriggerCallback('ctn-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    smashVitrine(case)
                                                else
                                                    QBCore.Functions.Notify('Your weapon doesn\'t seem strong enough ..', 'error')
                                                end
                                            else
...
function smashVitrine(k)
    ...
    QBCore.Functions.Progressbar("smash_vitrine", "Robbing", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('ctn-jewellery:server:setVitrineState', "isOpened", true, k)
        TriggerServerEvent('ctn-jewellery:server:setVitrineState', "isBusy", false, k)
        TriggerServerEvent('ctn-jewellery:server:vitrineReward')
        TriggerServerEvent('ctn-jewellery:server:setTimeout')
        TriggerEvent('dispatch:jewelryRobbery')
        local cameraID = math.random(31,34)
        if not usedCameras[cameraID] then
            usedCameras[cameraID] = true
    ...
    TriggerServerEvent('ctn-jewellery:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('ctn-jewellery:client:setVitrineState')
AddEventHandler('ctn-jewellery:client:setVitrineState', function(stateType, state, k)
    Config.Locations[k][stateType] = state
end)

RegisterNetEvent('ctn-jewellery:client:resetCameras')
AddEventHandler('ctn-jewellery:client:resetCameras', function()
    usedCameras = {}
end)

RegisterNetEvent('ctn-jewellery:client:setAlertState')
AddEventHandler('ctn-jewellery:client:setAlertState', function(bool)
    robberyAlert = bool
end)
