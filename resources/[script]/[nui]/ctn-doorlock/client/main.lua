				...
				if QB.Doors[k].pickable then
					if QB.Doors[k].locked then
						if hasItem then
							...
							TriggerEvent('ctn-lockpick:client:openLockpick', lockpickFinish)
						else
							QBCore.Functions.Notify("You are missing a toolkit..", "error")
						end
					else
						QBCore.Functions.Notify('door is already unlocked??', 'error', 2500)
					end
				else
					QBCore.Functions.Notify('The door has too strong a lock', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function setDoorLocking(doorId, key)
    ...
    SetTimeout(400, function()
		...
		TriggerServerEvent('ctn-doorlock:server:updateState', key, doorId.locked)
	end)
end
...
RegisterNetEvent('ctn-doorlock:client:setState')
AddEventHandler('ctn-doorlock:client:setState', function(doorID, state)
	QB.Doors[doorID].locked = state
end)

RegisterNetEvent('ctn-doorlock:client:setDoors')
AddEventHandler('ctn-doorlock:client:setDoors', function(doorList)
	QB.Doors = doorList
end)

RegisterNetEvent('QBCore:Client:OnPlayerLoaded')
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ctn-doorlock:server:setupDoors")
end)