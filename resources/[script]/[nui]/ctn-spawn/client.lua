...
RegisterNetEvent('ctn-spawn:client:openUI')
AddEventHandler('ctn-spawn:client:openUI', function(value)
...
RegisterNUICallback('spawnplayer', function(data)
    ...
    if type == "current" then
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('ctn-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('ctn-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
        FreezeEntityPosition(ped, false)
    ...
    elseif type == "house" then
        ...
        DoScreenFadeOut(500)
        Citizen.Wait(2000)
        TriggerEvent('ctn-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
        TriggerServerEvent('ctn-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('ctn-apartments:server:SetInsideMeta', 0, 0, false)
        FreezeEntityPosition(ped, false)
        RenderScriptCams(false, true, 500, true, true)
        SetCamActive(cam, false)
        DestroyCam(cam, true)
...
RegisterNetEvent('ctn-spawn:client:setupSpawns')
AddEventHandler('ctn-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        QBCore.Functions.TriggerCallback('ctn-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
