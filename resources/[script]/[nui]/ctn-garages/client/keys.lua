...
Citizen.CreateThread(function()
    while true do
        ...
        if not HasKey and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and QBCore ~= nil and not IsHotwiring then
            ...
            QBCore.Functions.DrawText3D(vehpos.x, vehpos.y, vehpos.z, "[G] Search" )
            SetVehicleEngineOn(veh, false, false, true)
        ...
        QBCore.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
            if result then
                ...
                if not isParked then
                    ...
                    QBCore.Functions.Notify('You can\'t use open your vehicle while its parked.', 'error')
                end
            else
                QBCore.Functions.Notify("No keys for target vehicle!", 'error')
            end
    ...
    if not HasKey then 
        ...
        if vehicle ~= nil and vehicle ~= 0 then
            if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
                ...
                if exports["ctn-taskbarskill"]:taskBar(math.random(5000,25000),math.random(10,20)) ~= 100 then             
                    ...
                    if c == o then
                        TriggerServerEvent('ctn-hud:Server:GainStress', math.random(1, 4))
                    end
                    return
                end
                ...
                if exports["ctn-taskbarskill"]:taskBar(1500,math.random(5,15)) ~= 100 then
                    StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
                    HasKey = false
                    SetVehicleEngineOn(vehicle, false, false, true)
                    QBCore.Functions.Notify("Lockpicking failed!", "error")
                ...
                IsHotwiring = false
                TriggerServerEvent('ctn-hud:Server:GainStress', math.random(2, 4))
    ...
    if not HasKey then 
        ...
        if not isParked then
            if vehicleHotwired[GetVehicleNumberPlateText(vehicle)] then
                QBCore.Functions.Notify("You can not work out this hotwire.", "error")
                return
            end
...
function isParked(plate)
    for name, data in pairs(CTNGarages.Config['garages']) do
            ...
            if aiming then
                if DoesEntityExist(targetPed) and not IsPedAPlayer(targetPed) and IsPedArmed(PlayerPedId(), 7) and IsPedInAnyVehicle(targetPed, false) then
                    ...
                    if not IsEntityDead(targetPed) and distance < 5 then
                        ...
                        Wait(750)
                        QBCore.Functions.Notify('You just recieved keys to a vehicle!')
                        TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle), vehicle)
                        Citizen.Wait(500)
...
function IsBlacklistedWeapon()
    ...
    if weapon ~= nil then
        for _, v in pairs(CTNGarages.Config['settings']['blacklistedWeapons']) do
