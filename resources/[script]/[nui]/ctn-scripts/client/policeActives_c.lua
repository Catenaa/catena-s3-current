...
RegisterNetEvent('ctn-policeActives:client:createNUI')
AddEventHandler('ctn-policeActives:client:createNUI', function(data, draggable)
    local html = ""
...
AddEventHandler("QBCore:Client:OnJobUpdate", function(JobInfo)
    TriggerServerEvent('ctn-policeActives:server:updateOfficers')

    if isOpened and (JobInfo.name ~= 'police' or JobInfo.name ~= 'gendarme') then
        SendNUIMessage({
            type = 'close',
        })
    end
...
RegisterNetEvent('ctn-policeActives:client:updateOfficers')
AddEventHandler('ctn-policeActives:client:updateOfficers', function(data)
    if not QBCore or not QBCore.Functions.GetPlayerData().job or QBCore.Functions.GetPlayerData().job.name ~= 'police' or QBCore.Functions.GetPlayerData().job.name ~= 'gendarme' then
        return
    end
