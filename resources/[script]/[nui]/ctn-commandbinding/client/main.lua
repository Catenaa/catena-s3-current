...
RegisterNetEvent('ctn-commandbinding:client:openUI')
AddEventHandler('ctn-commandbinding:client:openUI', function()
    openBindingMenu()
end)
                ...
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    ...
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            QBCore.Functions.Notify('There is still nothing binded to ['..v..'] , /binds to bind a command', 'primary', 4000)
                        end
                    else
                        QBCore.Functions.Notify('You don\'t have any commands yet, /binds to bind a command', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    QBCore.Functions.Notify('Commandbindings are stored!', 'success')

    TriggerServerEvent('ctn-commandbinding:server:setKeyMeta', keyData)
end)