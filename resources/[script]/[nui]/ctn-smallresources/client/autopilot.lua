...
function auto(speed)
	...
	if speed ~= 'stop' then
		autop = true
		QBCore.Functions.Notify("Autopilot Enabled. Press [S] to disable", "success")
		TaskVehicleDriveToCoord(player, pedveh, idcoord.x, idcoord.y, idcoord.z, pedspeed, 156, pedveh, drivestyle, 5.5, true)
	else
		QBCore.Functions.Notify("Autopilot Disabled", "error")
...
RegisterCommand("autopilot", function(source, args, rawCommand)
	...
	if IsPedInAnyVehicle(player) then
		...
		if canStart then
			...
			QBCore.Functions.Notify("This car does not have autopilot", "error")
		end					
	else
