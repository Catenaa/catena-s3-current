...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function(data)
    QBCore.Functions.TriggerCallback('ctn-banking:server:requestBanks', function(banksdata)
        banks = banksdata
        playerData = data
        playerLoaded = true
        createBlips()
...
RegisterNetEvent('ctn-banking:client:syncBanks')
AddEventHandler('ctn-banking:client:syncBanks', function(data)
    banks = data
    if showing then
        showing = false
    end
...
RegisterNetEvent('ctn-banking:updateCash')
AddEventHandler('ctn-banking:updateCash', function(data)
...
function openAccountScreen()
    QBCore.Functions.TriggerCallback('ctn-banking:getBankingInformation', function(banking)
        if banking ~= nil then
            InBank = true
...
function atmRefresh()
    QBCore.Functions.TriggerCallback('ctn-banking:getBankingInformation', function(infor)
        InBank = true
        SetNuiFocus(true, true)
        SendNUIMessage({
            status = "refreshatm",
...
RegisterNetEvent('ctn-banking:openBankScreen')
AddEventHandler('ctn-banking:openBankScreen', function()
    openAccountScreen()
end)

RegisterNetEvent('ctn-banking:client:usedMoneyBag')
AddEventHandler('ctn-banking:client:usedMoneyBag', function(item)
        ...
        if v.bankType == "Paleto" and v.moneyBags ~= nil and v.bankOpen then
            ...
            if moneyBagDist < 1.0 then
                QBCore.Functions.Progressbar("accessing_atm", "Cashier Counting Bag..", 60000, false, true, {
                    ...
                    TriggerServerEvent('ctn-banking:server:unpackMoneyBag', item)
                end, function()
...
RegisterNetEvent('ctn-banking:transferError')
AddEventHandler('ctn-banking:transferError', function(msg)
    SendNUIMessage({
...
RegisterNetEvent('ctn-banking:successAlert')
AddEventHandler('ctn-banking:successAlert', function(msg)
    SendNUIMessage({
