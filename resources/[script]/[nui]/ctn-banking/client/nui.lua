...
RegisterNetEvent('ctn-banking:client:newCardSuccess')
AddEventHandler('ctn-banking:client:newCardSuccess', function(cardno, ctype)
    SendNUIMessage({
        status = "updateCard",
        number = cardno,
        cardtype = ctype
...
RegisterNUICallback("createSavingsAccount", function(data, cb)
    TriggerServerEvent('ctn-banking:createSavingsAccount')
...
RegisterNUICallback("doDeposit", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        ...
        TriggerServerEvent('ctn-banking:doQuickDeposit', data.amount)
        openAccountScreen()
    end
end)
...
RegisterNUICallback("doWithdraw", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        ...
        TriggerServerEvent('ctn-banking:doQuickWithdraw', data.amount, true)
        openAccountScreen()
    end
end)
...
RegisterNUICallback("doATMWithdraw", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        ...
        TriggerServerEvent('ctn-banking:doQuickWithdraw', data.amount, false)
        openAccountScreen()
    end
end)
...
RegisterNUICallback("doTransfer", function(data, cb)
    if data ~= nil then
        TriggerServerEvent('ctn-banking:initiateTransfer', data)
    end
end)

RegisterNUICallback("createDebitCard", function(data, cb)
    if data.pin ~= nil then
        TriggerServerEvent('ctn-banking:createBankCard', data.pin)
    end
end)

RegisterNUICallback("lockCard", function(data, cb)
    TriggerServerEvent('ctn-banking:toggleCard', true)
end)

RegisterNUICallback("unLockCard", function(data, cb)
    TriggerServerEvent('ctn-banking:toggleCard', false)
end)

RegisterNUICallback("updatePin", function(data, cb)
    if data.pin ~= nil then 
        TriggerServerEvent('ctn-banking:updatePin', data.pin)
