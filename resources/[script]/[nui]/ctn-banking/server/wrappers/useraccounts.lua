function generateCurrent(cid)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bank_cards` WHERE `citizenid` = '" .. self.cid .."'", function(getCurrentAccount)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bank_statements` WHERE `account` = 'Current' AND `citizenid` = '" .. self.cid .. "' ORDER BY `record_id` DESC LIMIT 30", function(bankStatement)
        ...
        if not self.cardActive then
            ...
            QBCore.Functions.ExecuteSql(false, "UPDATE `bank_cards` SET `cardNumber` = '" .. cardNumber .. "', `cardPin` = '" .. pinSet .. "', `cardDecrypted` = 'false', `cardActive` = 1, `cardLocked` = 0, `cardType` = '" .. friendlyName .. "' WHERE `citizenid` = '" ..self.cid .. "' AND `record_id` = '" .. self.aid .. "'", function(rowsChanged)
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('ctn-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
...
RegisterServerEvent('ctn-banking:server:registerCurrentAccount')
AddEventHandler('ctn-banking:server:registerCurrentAccount', function(cid)
...
function generateSavings(cid)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bank_accounts` WHERE `citizenid` = '" .. self.cid .."' AND `account_type` = 'Savings'", function(getSavingsAccount)
        if getSavingsAccount[1] ~= nil then
            self.aid = getSavingsAccount[1].record_id
            self.balance = getSavingsAccount[1].amount
        end
        
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bank_statements` WHERE `account` = 'Savings' AND `citizenid` = '" .. self.cid .. "' ORDER BY `record_id` DESC LIMIT 30", function(stats)
    ...
    rTable.AddMoney = function(amt, text)
        if type(amt) == "number" and text then
            ...
            local time = os.date("%Y-%m-%d %H:%M:%S")
            QBCore.Functions.ExecuteSql(false, "INSERT INTO `bank_statements` (`citizenid`,  `account`, `deposited`, `withdraw`, `balance`, `date`, `type`) VALUES ('" .. self.cid .. "', 'Saving','" .. amt .. "', '', '" .. self.balance .. "', '" .. time .. "', '" .. text .. "')")
...
RegisterServerEvent('ctn-banking:server:registerSavingsAccount')
AddEventHandler('ctn-banking:server:registerSavingsAccount', function(cid)
