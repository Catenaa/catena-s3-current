...
Citizen.CreateThread(function()
    ...
    QBCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_type` = 'Buisness'", function(accts)
        buis = #accts
        if accts[1] ~= nil then
...
RegisterServerEvent('ctn-banking:server:modifyBank')
AddEventHandler('ctn-banking:server:modifyBank', function(bank, k, v)
    if banks[tonumber(bank)] then
        banks[tonumber(bank)][k] = v
        TriggerClientEvent('ctn-banking:client:syncBanks', -1, banks)
    end
end)

exports('modifyBank', function(bank, k, v)
    TriggerEvent('ctn-banking:server:modifyBank', bank, k, v)
end)
...
QBCore.Functions.CreateCallback('ctn-banking:server:requestBanks', function(source, cb)
    repeat Wait(0) until banks ~= nil
    cb(banks)
end)

QBCore.Functions.CreateCallback('ctn-banking:server:checkMoneyBagCount', function(source, cb)
    local src = source
...
RegisterServerEvent('ctn-banking:createNewCard')
AddEventHandler('ctn-banking:createNewCard', function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    TriggerEvent('ctn-logs:server:createLog', 'banking', 'Banking', "Created new card **[" .. xPlayer.PlayerData.citizenid .. "]**", src)
end)

RegisterServerEvent('rl-base:itemUsed')
AddEventHandler('rl-base:itemUsed', function(_src, data)
    if data.item == "moneybag" then
        TriggerClientEvent('ctn-banking:client:usedMoneyBag', _src, data)
    end
end)

RegisterServerEvent('ctn-banking:server:unpackMoneyBag')
AddEventHandler('ctn-banking:server:unpackMoneyBag', function(item)
    local _src = source
    if item ~= nil then
...
function getCharacterName(cid)
    local name
    MySQL.Async.fetchAll("SELECT * FROM `playerskins` WHERE `cid` = @cid", {['@cid'] = cid}, function(exists)
        if exists[1] ~= nil then 
...
RegisterServerEvent('ctn-banking:initiateTransfer')
AddEventHandler('ctn-banking:initiateTransfer', function(data)
    --[[
    local _src = source
    ...
    if (checkAccount) then 
        ...
        if receiptName ~= false or receiptName ~= nil then 
            ...
            if userOnline ~= false then
                ...
                if acType == "Current" then
                    ...
                    TriggerClientEvent('ctn-banking:openBankScreen', _src)
                    TriggerClientEvent('ctn-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('ctn-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
QBCore.Functions.CreateCallback('ctn-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-banking:createBankCard')
AddEventHandler('ctn-banking:createBankCard', function(pin)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('ctn-banking:openBankScreen', src)
    TriggerClientEvent('QBCore:Notify', src, 'You have successfully ordered a Debit Card.', 'success')
    
    TriggerEvent('ctn-logs:server:createLog', 'banking', 'Banking', 'Successfully ordered a Debit Card', src)
end)

RegisterServerEvent('ctn-banking:doQuickDeposit')
AddEventHandler('ctn-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('ctn-banking:openBankScreen', _src)
            TriggerClientEvent('ctn-banking:successAlert', _src, 'You made a cash deposit of $'..amount..' successfully.')
            TriggerEvent('ctn-logs:server:createLog', 'banking', 'Banking', 'Made a cash deposit of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('ctn-banking:toggleCard')
AddEventHandler('ctn-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-banking:doQuickWithdraw')
AddEventHandler('ctn-banking:doQuickWithdraw', function(amount, branch)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then 
            TriggerClientEvent('ctn-banking:openBankScreen', _src)
            TriggerClientEvent('ctn-banking:successAlert', _src, 'You made a cash withdrawal of $'..amount..' successfully.')
            TriggerEvent('ctn-logs:server:createLog', 'banking', 'Banking', 'Made a cash withdrawal of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('ctn-banking:updatePin')
AddEventHandler('ctn-banking:updatePin', function(pin)
    if pin ~= nil then 
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        TriggerClientEvent('ctn-banking:openBankScreen', src)
        TriggerClientEvent('ctn-banking:successAlert', src, 'You have successfully updated your Debit card pin.')
    end
end)

RegisterServerEvent('ctn-banking:savingsDeposit')
AddEventHandler('ctn-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('ctn-banking:openBankScreen', src)
        TriggerClientEvent('ctn-banking:successAlert', src, 'You made a savings deposit of $'..tostring(amount)..' successfully.')
        TriggerEvent('ctn-logs:server:createLog', 'banking', 'Banking', 'made a savings deposit of $'..tostring(amount)..' successfully..', src)
    end
end)

RegisterServerEvent('ctn-banking:savingsWithdraw')
AddEventHandler('ctn-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('ctn-banking:openBankScreen', src)
        TriggerClientEvent('ctn-banking:successAlert', src, 'You made a savings withdrawal of $'..tostring(amount)..' successfully.')
        TriggerEvent('ctn-logs:server:createLog', 'banking', 'Banking', 'Made a savings withdrawal of $'..tostring(amount)..' successfully.', src)
    end
end)

RegisterServerEvent('ctn-banking:createSavingsAccount')
AddEventHandler('ctn-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ctn-banking:openBankScreen', src)
    TriggerClientEvent('ctn-banking:successAlert', src, 'You have successfully opened a savings account.')
    TriggerEvent('ctn-logs:server:createLog', 'banking', 'Banking', "Created new saving account", src)
end)
