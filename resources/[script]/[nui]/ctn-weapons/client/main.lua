    ...
    if CurrentWeaponData ~= nil then
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and QBCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if newtotal < max then
                QBCore.Functions.Progressbar("taking_bullets", "Reloading", 5000, false, true, {
                    ...
                    if QBCore.Shared.Weapons[weapon] ~= nil then
                        ...
                        TriggerEvent('QBCore:Notify', clipmax .." bullets loaded!", "success")
                ...
                QBCore.Functions.Notify("The ammo for this weapon is full..", "error")
            end
        else    
...
AddEventHandler('ctn-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        if weapon ~= nil then
            ...
            QBCore.Functions.Notify('You dont have a weapon..', 'error')
        end
    end
end)
...
AddEventHandler("weapons:client:EquipAttachment", function(ItemData, attachment)
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        QBCore.Functions.Notify("You have no weapon in your hand..", "error")
    end
end)
