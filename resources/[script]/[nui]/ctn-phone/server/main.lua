...
RegisterServerEvent('ctn-phone:server:AddAdvert')
AddEventHandler('ctn-phone:server:AddAdvert', function(msg)
    local src = source
    ...
    TriggerClientEvent('ctn-phone:client:UpdateAdverts', -1, Adverts, "@"..Player.PlayerData.charinfo.firstname..""..Player.PlayerData.charinfo.lastname)
end)
...
RegisterServerEvent('qb-phone:server:OpenCamera')
AddEventHandler('qb-phone:server:OpenCamera', function()
    local src = source
    TriggerClientEvent('camera:open', src)
end)

QBCore.Functions.CreateCallback('ctn-phone:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
        ...
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM player_contacts WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ORDER BY `name` ASC", function(result)
            ...
            QBCore.Functions.ExecuteSql(false, "SELECT * FROM phone_invoices WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(invoices)
                ...
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `ctnvehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(garageresult)
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetCallState', function(source, cb, ContactData)
...
RegisterServerEvent('ctn-phone:server:SetCallState')
AddEventHandler('ctn-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterServerEvent('ctn-phone:server:RemoveMail')
AddEventHandler('ctn-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    SetTimeout(100, function()
        QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(mails)
            ...
            TriggerClientEvent('ctn-phone:client:UpdateMails', src, mails)
        end)
    end)
end)
...
RegisterServerEvent('ctn-phone:server:sendNewMail')
AddEventHandler('ctn-phone:server:sendNewMail', function(mailData, srcz)
    local src = source
    ...
    TriggerClientEvent('ctn-phone:client:NewMailNotify', src, mailData)
...
RegisterServerEvent('ctn-phone:server:sendNewMailToOffline')
AddEventHandler('ctn-phone:server:sendNewMailToOffline', function(citizenid, mailData)
...
RegisterServerEvent('ctn-phone:server:sendNewEventMail')
AddEventHandler('ctn-phone:server:sendNewEventMail', function(citizenid, mailData)
    if mailData.button == nil then
        QBCore.Functions.ExecuteSql(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES ('"..citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0')")
    else
        QBCore.Functions.ExecuteSql(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`, `button`) VALUES ('"..citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0', '"..json.encode(mailData.button).."')")
...
RegisterServerEvent('ctn-phone:server:ClearButtonData')
AddEventHandler('ctn-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:MentionedPlayer')
AddEventHandler('ctn-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('ctn-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
...
RegisterServerEvent('ctn-phone:server:CallContact')
AddEventHandler('ctn-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(TargetData.number)

    if Target ~= nil then
        TriggerClientEvent('ctn-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

QBCore.Functions.CreateCallback('ctn-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Invoices = {}

    Ply.Functions.RemoveMoney('bank', amount, "paid-invoice")
    TriggerEvent("ctn-bossmenu:server:addAccountMoney", society, amount)
...
QBCore.Functions.CreateCallback('ctn-phone:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:UpdateHashtags')
AddEventHandler('ctn-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('ctn-phone:client:UpdateHashtags', -1, Handle, messageData)
end)
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetContactPictures', function(source, cb, Chats)
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetContactPicture', function(source, cb, Chat)
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetPicture', function(source, cb, number)
...
RegisterServerEvent('ctn-phone:server:SetPhoneAlerts')
AddEventHandler('ctn-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = QBCore.Functions.GetPlayer(src).citizenid
    RLPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterServerEvent('ctn-phone:server:UpdateTweets')
AddEventHandler('ctn-phone:server:UpdateTweets', function(NewTweets, TweetData)
    Tweets = NewTweets
    local TwtData = TweetData
    local src = source
    TriggerClientEvent('ctn-phone:client:UpdateTweets', -1, src, Tweets, TwtData)
end)

RegisterServerEvent('ctn-phone:server:TransferMoney')
AddEventHandler('ctn-phone:server:TransferMoney', function(iban, amount)
    local src = source
        ...
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                if PhoneItem ~= nil then
                    TriggerClientEvent('ctn-phone:client:TransferMoney', recieverSteam.PlayerData.source, amount, recieverSteam.PlayerData.money.bank)
                end
            else
...
RegisterServerEvent('ctn-phone:server:EditContact')
AddEventHandler('ctn-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:RemoveContact')
AddEventHandler('ctn-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:AddNewContact')
AddEventHandler('ctn-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:UpdateMessages')
AddEventHandler('ctn-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
            ...
            if TargetData ~= nil then
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        TriggerClientEvent('ctn-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
                    else
                        -- Insert for target
                        ...
                        -- Send notification & Update messages for target
                        TriggerClientEvent('ctn-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
                    end
                end)
            else
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
...
RegisterServerEvent('ctn-phone:server:AddRecentCall')
AddEventHandler('ctn-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ctn-phone:client:AddRecentCall', src, data, label, type)

    local Trgt = QBCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('ctn-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterServerEvent('ctn-phone:server:CancelCall')
AddEventHandler('ctn-phone:server:CancelCall', function(ContactData)
    local Ply = QBCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('ctn-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('ctn-phone:server:AnswerCall')
AddEventHandler('ctn-phone:server:AnswerCall', function(CallData)
    local Ply = QBCore.Functions.GetPlayerByPhone(CallData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('ctn-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('ctn-phone:server:SaveMetaData')
AddEventHandler('ctn-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-phone:server:FetchResult', function(source, cb, search)
    local src = source
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
    QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `ctnvehicles` WHERE `plate` LIKE "%'..search..'%" OR `citizenid` = "'..search..'"', function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
...
QBCore.Functions.CreateCallback('ctn-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then 
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetGarageVehicles', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    local Vehicles = {}

    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `ctnvehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                local VehicleData = QBCore.Shared.Vehicles[v.model]
...
QBCore.Functions.CreateCallback('ctn-phone:server:HasPhone', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateCallback('ctn-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('ctn-phone:server:GiveContactDetails')
AddEventHandler('ctn-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ctn-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterServerEvent('ctn-phone:server:AddTransaction')
AddEventHandler('ctn-phone:server:AddTransaction', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetCurrentDrivers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('ctn-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
RegisterServerEvent('ctn-phone:server:InstallApplication')
AddEventHandler('ctn-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:restoreRented')
AddEventHandler('ctn-phone:server:restoreRented', function(money)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:removeMoney')
AddEventHandler('ctn-phone:server:removeMoney', function(type, money)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:RemoveInstallation')
AddEventHandler('ctn-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone:server:CallDriver')
AddEventHandler('ctn-phone:server:CallDriver', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent("ctn-phone:client:AddTaxiCall", -1, CurrentCallID, TaxiCalls[CurrentCallID])
end)

RegisterServerEvent('ctn-phone:server:AcceptDriverCall')
AddEventHandler('ctn-phone:server:AcceptDriverCall', function(ID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if TaxiCalls[ID] then
        TriggerClientEvent("ctn-phone:client:RemoveTaxiCall", -1, ID)
        TriggerClientEvent("ctn-phone:client:AcceptDriverCall", -1, TaxiCalls[ID])
...
RegisterServerEvent('ctn-phone:server:spawnVehicle')
AddEventHandler('ctn-phone:server:spawnVehicle', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Config.RentelVehicles[data.model] and data.price <= Player['PlayerData']['money']['cash'] then
        ...
        TriggerClientEvent('ctn-phone:client:spawnVehicle', src, data)
        TriggerEvent('ctn-phone:server:clearVehicleTrunk', data['plate'])
    else
        TriggerClientEvent('QBCore:Notify', src, "You don't have enough money.", 'error')
    end
end)

RegisterServerEvent('ctn-phone:server:clearVehicleTrunk')
AddEventHandler('ctn-phone:server:clearVehicleTrunk', function(plate)
