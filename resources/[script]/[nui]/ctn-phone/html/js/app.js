        ...
        if (CanOpenApp) {
            if (RL.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $.post('http://ctn-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                        RL.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                    })
                    $.post('http://ctn-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                        RL.Phone.Notifications.LoadHashtags(Hashtags)
                    })
                    if (RL.Phone.Data.IsOpen) {
                        $.post('http://ctn-phone/GetTweets', JSON.stringify({}), function(Tweets){
                            RL.Phone.Notifications.LoadTweets(Tweets);
                        });
                    }
                } else if (PressedApplication == "bank") {
                    $.post('http://ctn-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        RL.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('http://ctn-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        RL.Phone.Functions.LoadBankInvoices(invoices);
                    });

                    RL.Phone.Functions.DoBankOpen();
                } else if (PressedApplication == "whatsapp") {
                    $.post('http://ctn-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        RL.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('http://ctn-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        RL.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('http://ctn-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        RL.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('http://ctn-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('http://ctn-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('http://ctn-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        RL.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('http://ctn-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('http://ctn-phone/GetCryptoData', JSON.stringify({
                        crypto: "qbit",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('http://ctn-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                        RefreshCryptoTransactions(data);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('http://ctn-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });
                    ...
                    $.post('http://ctn-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('http://ctn-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    $.post('http://ctn-phone/GetCurrentLawyers', JSON.stringify({}), function(data){
                        SetupLawyers(data);
                    });
                } else if (PressedApplication == "rentel") {
                    $.post('http://ctn-phone/SetupRentel', JSON.stringify({}), function(data){
                        SetupRentel(data); 
                    });
                } else if (PressedApplication == "sellix") {
                    $.post('http://ctn-phone/SetupSellix', JSON.stringify({}), function(data){
                        SetupSellixData(data); 
                    });
                } else if (PressedApplication == "drivers") {
                    $.post('http://ctn-phone/GetCurrentDrivers', JSON.stringify({}), function(data){
                        SetupDrivers(data.drivers, data.isTaxi);
                    });
                } else if (PressedApplication == "store") {
                    $.post('http://ctn-phone/SetupStoreApps', JSON.stringify({}), function(data){
                        SetupAppstore(data); 
                    });

                } else if (PressedApplication == "trucker") {
                    $.post('http://ctn-phone/OpenCamera');
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('http://ctn-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
RL.Phone.Functions.Close = function() {
    ...
    } else if (RL.Phone.Data.currentApplication == "trucker") {
        RL.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
            RL.Phone.Animations.TopSlideUp('.'+RL.Phone.Data.currentApplication+"-app", 400, -160);
            $(".trucker-app").css({"display":"none"});
            RL.Phone.Functions.HeaderTextColor("white", 300);
            RL.Phone.Data.currentApplication = null;
    }

    RL.Phone.Animations.BottomSlideDown('.container', 300, -70);
    $.post('http://ctn-phone/Close');
...
RL.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('http://ctn-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
