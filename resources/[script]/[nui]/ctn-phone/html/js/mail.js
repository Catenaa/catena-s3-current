...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('http://ctn-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('http://ctn-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
RL.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(RL.Phone.Data.PlayerData.charinfo.firstname+"."+RL.Phone.Data.PlayerData.charinfo.lastname+"@catena.ma");
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('http://ctn-phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
