...
function SpawnVehicle(data)
    TriggerServerEvent('ctn-phone:server:spawnVehicle', data)
    Wait(1500)
end

RegisterNetEvent('ctn-phone:client:spawnVehicle')
AddEventHandler('ctn-phone:client:spawnVehicle', function(data)
    CloseMenu()
        
    if Config.RentelVehicles[data.model]['stored'] then
        Config.RentelVehicles[data.model]['stored'] = false
    else
        TriggerServerEvent('ctn-phone:server:removeMoney', 'cash', data.price)
    end
...
function RestoreVehicle(model)
    ...
    if Config.RentelVehicles[model] and Config.RentelVehicles[model]['stored'] then
        ...
        TriggerServerEvent('ctn-phone:server:restoreRented', math.floor(Config.RentelVehicles[model]['price'] / 2))
    end
end
...
function RentVehicle(model)
    ...
    if Config.RentelVehicles[model] and not Config.RentelVehicles[model]['stored'] and location and Config.RentelVehicles[model].price <= QBCore.Functions.GetPlayerData()['money']['bank'] then
        TriggerServerEvent('ctn-phone:server:removeMoney', 'bank', Config.RentelVehicles[model]['price'])
