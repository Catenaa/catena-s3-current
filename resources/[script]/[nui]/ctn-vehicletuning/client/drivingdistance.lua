                ...
                if seat == ped then
                    if not CheckDone then
                        if vehiclemeters == -1 then
                            CheckDone = true
                            QBCore.Functions.TriggerCallback('ctn-vehicletuning:server:IsVehicleOwned', function(IsOwned)
                                if IsOwned then
                ...
                if vehiclemeters ~= -1 then
                    if seat == ped then
                        if previousvehiclepos ~= nil then
                            ...
                            if DamageKey ~= nil then
                                ...
                                if chance == odd then
                                    for k, v in pairs(Config.Damages) do
                                        ...
                                        TriggerServerEvent('ctn-vehicletuning:server:SetPartLevel', plate, k, newDamage)
                                    end
                                end
                            end
                            ...
                            TriggerEvent('ctn-hud:client:UpdateDrivingMeters', true, amount)
                            TriggerServerEvent('ctn-vehicletuning:server:UpdateDrivingDistance', DrivingDistance[plate], plate)
                        end
                    else
                        if invehicle then
                            if DrivingDistance[plate] ~= nil then
...
RegisterNetEvent('ctn-vehicletuning:client:UpdateDrivingDistance')
AddEventHandler('ctn-vehicletuning:client:UpdateDrivingDistance', function(amount, plate)
