...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    isLoggedIn = true
    QBCore.Functions.TriggerCallback('ctn-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    QBCore.Functions.TriggerCallback('ctn-vehicletuning:server:GetDrivingDistances', function(retval)
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Retire")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Enter the service")
                        end
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("QBCore:ToggleDuty")
                        end
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('ctn-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            QBCore.Functions.Notify("You cannot put bicycles on the plate!", "error")
                                        end
                                    end
                                end
                            end
...
function RepairPart(part)
    ...
    QBCore.Functions.TriggerCallback('ctn-inventory:server:GetStashItems', function(StashItems)
            ...
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    QBCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." repairing", math.random(5000, 10000), false, true, {
                        ...
                        TriggerEvent('ctn-vehicletuning:client:RepaireeePart', part)
                        TriggerServerEvent('ctn-inventory:server:SaveStashItems', "mechanicstash", StashItems)
...
RegisterNetEvent('ctn-vehicletuning:client:RepaireeePart')
AddEventHandler('ctn-vehicletuning:client:RepaireeePart', function(part)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('ctn-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('ctn-vehicletuning:client:SetAttachedVehicle')
AddEventHandler('ctn-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            QBCore.Functions.Progressbar("repair_advanced", "Repairing..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['ctn-companies']:IsEmployee(v)
    end
    return retval
end
