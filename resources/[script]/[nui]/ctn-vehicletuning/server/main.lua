...
QBCore.Functions.CreateCallback('ctn-vehicletuning:server:GetDrivingDistances', function(source, cb)
...
RegisterServerEvent('ctn-vehicletuning:server:UpdateDrivingDistance')
AddEventHandler('ctn-vehicletuning:server:UpdateDrivingDistance', function(amount, plate)
    VehicleDrivingDistance[plate] = amount

    TriggerClientEvent('ctn-vehicletuning:client:UpdateDrivingDistance', -1, VehicleDrivingDistance[plate], plate)
...
QBCore.Functions.CreateCallback('ctn-vehicletuning:server:IsVehicleOwned', function(source, cb, plate)
    local retval = false
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `player_vehicles` WHERE `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
            retval = true
...
RegisterServerEvent('ctn-vehicletuning:server:LoadStatus')
AddEventHandler('ctn-vehicletuning:server:LoadStatus', function(veh, plate)
...
RegisterServerEvent('ctn-vehicletuning:server:SetPartLevel')
AddEventHandler('ctn-vehicletuning:server:SetPartLevel', function(plate, part, level)
    if VehicleStatus[plate] ~= nil then
        VehicleStatus[plate][part] = level
        TriggerClientEvent("vehiclemod:client:setVehicleStatus", -1, plate, VehicleStatus[plate])
    end
...
QBCore.Functions.CreateCallback('ctn-vehicletuning:server:GetAttachedVehicle', function(source, cb)
    cb(Config.Plates)
end)

QBCore.Functions.CreateCallback('ctn-vehicletuning:server:IsMechanicAvailable', function(source, cb)
	local amount = 0
...
RegisterServerEvent('ctn-vehicletuning:server:SetAttachedVehicle')
AddEventHandler('ctn-vehicletuning:server:SetAttachedVehicle', function(veh, k)
    if veh ~= false then
        Config.Plates[k].AttachedVehicle = veh
        TriggerClientEvent('ctn-vehicletuning:client:SetAttachedVehicle', -1, veh, k)
    else
        Config.Plates[k].AttachedVehicle = nil
        TriggerClientEvent('ctn-vehicletuning:client:SetAttachedVehicle', -1, false, k)
    end
end)

RegisterServerEvent('ctn-vehicletuning:server:CheckForItems')
AddEventHandler('ctn-vehicletuning:server:CheckForItems', function(part)
    local src = source
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            TriggerClientEvent('ctn-vehicletuning:client:RepaireeePart', src, part)
...
QBCore.Commands.Add("setmechanic", "Give the mechanic job", {{name="id", help="ID of the player"}}, false, function(source, args)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', source, "You hired ("..TargetData.PlayerData.charinfo.firstname..") as an Autocare employee!")
            end
        else
...
QBCore.Commands.Add("takemechanic", "Take someone's Mechanic job", {{name="id", help="ID of the player"}}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "mechanic" then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, "You fired ("..TargetData.PlayerData.charinfo.firstname..") as Autocare employee!")
                else
                    TriggerClientEvent('QBCore:Notify', source, "This is not an employee of Autocare!", "error")
                end
            end
        else
...
QBCore.Functions.CreateCallback('ctn-vehicletuning:server:GetStatus', function(source, cb, plate)
