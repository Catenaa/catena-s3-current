...
RegisterNetEvent('ctn-kidnapping:client:SetKidnapping')
AddEventHandler('ctn-kidnapping:client:SetKidnapping', function(bool)
    isKidnapping = bool
end)
...
RegisterNetEvent('ctn-trunk:client:KidnapTrunk')
AddEventHandler('ctn-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            if closestVehicle ~= 0 then
                ...
                Citizen.Wait(50)
                TriggerServerEvent("ctn-trunk:server:KidnapTrunk", GetPlayerServerId(closestPlayer), closestVehicle)
            end
        else
...
RegisterNetEvent('ctn-trunk:client:KidnapGetIn')
AddEventHandler('ctn-trunk:client:KidnapGetIn', function(veh)
    local ped = GetPlayerPed(-1)
    ...
    if TrunkClasses[vehClass].allowed then
        QBCore.Functions.TriggerCallback('ctn-trunk:server:getTrunkBusy', function(isBusy)
                ...
                if not inTrunk then
                    if not isBusy then
                        if not isKidnapped then
                            ...
                            TriggerServerEvent('ctn-trunk:server:setTrunkBusy', plate, true)
                            inTrunk = true
                            Citizen.Wait(500)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('ctn-smallresources:trunk:server:setTrunkBusy', plate, nil)
...
RegisterNetEvent('ctn-trunk:client:GetIn')
AddEventHandler('ctn-trunk:client:GetIn', function(isKidnapped)
    local ped = GetPlayerPed(-1)
    local closestVehicle = getNearestVeh()
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ClearPedTasks(ped)
                            inTrunk = false
                            TriggerServerEvent('ctn-trunk:server:setTrunkBusy', plate, false)
                            SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
                            SetEntityCollision(PlayerPedId(), true, true)
                            TrunkCam(false)
                        else
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('ctn-radialmenu:trunk:server:Door', false, plate, 5)
                            else
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('ctn-radialmenu:trunk:server:Door', true, plate, 5)
                            else
