...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "Bank account no. Copied", "#badc58", 1750);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
        if (amountData >= amount) {
            $.post('http://ctn-phone_new/TransferMoney', JSON.stringify({
                iban: iban,
                amount: amount
            }), function(data){
                if (data.CanTransfer) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "An amount has been transferred$ "+amount+",-!", "#badc58", 1500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "You don't have the required amount!", "#badc58", 1500);
                }
            })
            QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
        } else {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "Fill in all information!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://ctn-phone_new/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "you paid $"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
                ...
                QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "You do not have enough credit !", "#badc58", 1500);
            }
        });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "You do not have enough credit !", "#badc58", 1500);
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://ctn-phone_new/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        invoiceId: InvoiceData.invoiceid,
    }));
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "CTN Bank", "There is no bank account associated with this number", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});