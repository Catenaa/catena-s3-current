...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://ctn-phone_new/ClearRecentAlerts');
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('http://ctn-phone_new/CallContact', JSON.stringify({
        ContactData: cData,
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Phone", "You are already engaged");
                    }
                } else {
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        ...
        $("#total-contacts").text(TotalContacts+ " My contacts");
    } else {
        $("#total-contacts").text("0 My contacts");
    }
};
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            $.post('http://ctn-phone_new/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "You can't text yourself..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://ctn-phone_new/EditContact', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Contact Edit", "fill out all fields!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://ctn-phone_new/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://ctn-phone_new/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://ctn-phone_new/RemoveSuggestion', JSON.stringify({
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('http://ctn-phone_new/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('http://ctn-phone_new/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('http://ctn-phone_new/CancelOngoingCall');
});
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('http://ctn-phone_new/AnswerCall');
});
