        ...
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#mySerialNumber").text("ctn-" + QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                } else if (PressedApplication == "twitter") {
                    $.post('http://ctn-phone_new/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                        QB.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                    })
                    $.post('http://ctn-phone_new/GetHashtags', JSON.stringify({}), function(Hashtags){
                        QB.Phone.Notifications.LoadHashtags(Hashtags)
                    })
                    if (QB.Phone.Data.IsOpen) {
                        $.post('http://ctn-phone_new/GetTweets', JSON.stringify({}), function(Tweets){
                            QB.Phone.Notifications.LoadTweets(Tweets);
                        });
                    }
                } else if (PressedApplication == "bank") {
                    QB.Phone.Functions.DoBankOpen();
                    $.post('http://ctn-phone_new/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('http://ctn-phone_new/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('http://ctn-phone_new/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('http://ctn-phone_new/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('http://ctn-phone_new/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        QB.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('http://ctn-phone_new/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('http://ctn-phone_new/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('http://ctn-phone_new/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('http://ctn-phone_new/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('http://ctn-phone_new/GetCryptoData', JSON.stringify({
                        crypto: "A5bit",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('http://ctn-phone_new/GetCryptoTransactions', JSON.stringify({}), function(data){
                        RefreshCryptoTransactions(data);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('http://ctn-phone_new/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });
				} else if (PressedApplication == "taxis") {
                    $.post('http://ctn-phone_new/GetCurrentTaxi', JSON.stringify({}), function(data){
                        Setuptaxi(data);
                    });
                } else if (PressedApplication == "tows") {
                    $.post('http://ctn-phone_new/GetCurrentTow', JSON.stringify({}), function(data){
                        Setuptows(data);
                    });
				
                } else if (PressedApplication == "mechs") {
                    $.post('http://ctn-phone_new/GetCurrentMech', JSON.stringify({}), function(data){
                        Setupmechs(data);
                    });
				
				} else if (PressedApplication == "emergencys") {
                    $.post('http://ctn-phone_new/GetCurrentEmergency', JSON.stringify({}), function(data){
                        Setupemergencys(data);
                    });
                } else if (PressedApplication == "houses") {
                    $.post('http://ctn-phone_new/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('http://ctn-phone_new/GetPlayerKeys', JSON.stringify({}), function(Keys){
                        $(".house-app-mykeys-container").html("");
                        if (Keys.length > 0) {
                            $.each(Keys, function(i, key){
                                var elem = '<div class="mykeys-key" id="keyid-'+i+'"> <span class="mykeys-key-label">' + key.HouseData.adress + '</span> <span class="mykeys-key-sub">Click to specify the location</span> </div>';
                    ...
                    $.post('http://ctn-phone_new/GetCurrentLawyers', JSON.stringify({}), function(data){
                        SetupLawyers(data);
                    });
                } else if (PressedApplication == "store") {
                    $.post('http://ctn-phone_new/SetupStoreApps', JSON.stringify({}), function(data){
                        SetupAppstore(data); 
                    });
                } else if (PressedApplication == "trucker") {
                    $.post('http://ctn-phone_new/GetTruckerData', JSON.stringify({}), function(data){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('http://ctn-phone_new/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
QB.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('http://ctn-phone_new/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
