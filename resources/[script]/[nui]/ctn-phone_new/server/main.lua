...
RegisterServerEvent('ctn-phone_new:server:AddAdvert')
AddEventHandler('ctn-phone_new:server:AddAdvert', function(msg)
    local src = source
    ...
    TriggerClientEvent('ctn-phone_new:client:UpdateAdverts', -1, Adverts, "@"..Player.PlayerData.charinfo.firstname..""..Player.PlayerData.charinfo.lastname)
end)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetCallState', function(source, cb, ContactData)
...
RegisterServerEvent('ctn-phone_new:server:SetCallState')
AddEventHandler('ctn-phone_new:server:SetCallState', function(bool)
    local src = source
...
RegisterServerEvent('ctn-phone_new:server:RemoveMail')
AddEventHandler('ctn-phone_new:server:RemoveMail', function(MailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    SetTimeout(100, function()
        QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(mails)
            ...
            TriggerClientEvent('ctn-phone_new:client:UpdateMails', src, mails)
        end)
    end)
end)
...
AddEventHandler('ctn-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ctn-phone_new:client:NewMailNotify', src, mailData)
...
RegisterServerEvent('ctn-phone_new:server:ClearButtonData')
AddEventHandler('ctn-phone_new:server:ClearButtonData', function(mailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone_new:server:MentionedPlayer')
AddEventHandler('ctn-phone_new:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('ctn-phone_new:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
...
RegisterServerEvent('ctn-phone_new:server:CallContact')
AddEventHandler('ctn-phone_new:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(TargetData.number)

    if Target ~= nil then
        TriggerClientEvent('ctn-phone_new:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

QBCore.Functions.CreateCallback('ctn-phone_new:server:PayInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Trgt = QBCore.Functions.GetPlayerByCitizenId(sender)
    local Invoices = {}
...
RegisterServerEvent('ctn-phone_new:server:UpdateHashtags')
AddEventHandler('ctn-phone_new:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('ctn-phone_new:client:UpdateHashtags', -1, Handle, messageData)
end)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetContactPictures', function(source, cb, Chats)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetContactPicture', function(source, cb, Chat)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetPicture', function(source, cb, number)
...
RegisterServerEvent('ctn-phone_new:server:UpdateTweets')
AddEventHandler('ctn-phone_new:server:UpdateTweets', function(NewTweets, TweetData)
    Tweets = NewTweets
    local TwtData = TweetData
    local src = source
    TriggerClientEvent('ctn-phone_new:client:UpdateTweets', -1, src, Tweets, TwtData)
end)

RegisterServerEvent('ctn-phone_new:server:TransferMoney')
AddEventHandler('ctn-phone_new:server:TransferMoney', function(iban, amount)
    local src = source
        ...
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                if PhoneItem ~= nil then
                    TriggerClientEvent('ctn-phone_new:client:TransferMoney', recieverSteam.PlayerData.source, amount, recieverSteam.PlayerData.money.bank)
                end
            else
                local moneyInfo = json.decode(result[1].money)
...
RegisterServerEvent('ctn-phone_new:server:EditContact')
AddEventHandler('ctn-phone_new:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone_new:server:RemoveContact')
AddEventHandler('ctn-phone_new:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone_new:server:AddNewContact')
AddEventHandler('ctn-phone_new:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ctn-phone_new:server:UpdateMessages')
AddEventHandler('ctn-phone_new:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
            ...
            if TargetData ~= nil then
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        TriggerClientEvent('ctn-phone_new:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
                    else
                        -- Insert for target
                        ...
                        -- Send notification & Update messages for target
                        TriggerClientEvent('ctn-phone_new:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
                    end
                end)
            else
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
...
RegisterServerEvent('ctn-phone_new:server:AddRecentCall')
AddEventHandler('ctn-phone_new:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ctn-phone_new:client:AddRecentCall', src, data, label, type)

    local Trgt = QBCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('ctn-phone_new:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterServerEvent('ctn-phone_new:server:CancelCall')
AddEventHandler('ctn-phone_new:server:CancelCall', function(ContactData)
    local Ply = QBCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('ctn-phone_new:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('ctn-phone_new:server:AnswerCall')
AddEventHandler('ctn-phone_new:server:AnswerCall', function(CallData)
    local Ply = QBCore.Functions.GetPlayerByPhone(CallData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('ctn-phone_new:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('ctn-phone_new:server:SaveMetaData')
AddEventHandler('ctn-phone_new:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:FetchResult', function(source, cb, search)
    local src = source
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', src, "There is no nearby vehicle ..", "error")
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetGarageVehicles', function(source, cb)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:HasPhone', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:CanTransferMoney', function(source, cb, amount, iban)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('ctn-phone_new:server:GiveContactDetails')
AddEventHandler('ctn-phone_new:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ctn-phone_new:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterServerEvent('ctn-phone_new:server:AddTransaction')
AddEventHandler('ctn-phone_new:server:AddTransaction', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetCurrentTow', function(source, cb)
    local tow = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetCurrentMech', function(source, cb)
    local mech = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('ctn-phone_new:server:GetCurrentTaxi', function(source, cb)
    local taxi = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
RegisterServerEvent('ctn-phone_new:server:InstallApplication')
AddEventHandler('ctn-phone_new:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('ctn-phone_new:RefreshPhone', src)
end)

RegisterServerEvent('ctn-phone_new:server:RemoveInstallation')
AddEventHandler('ctn-phone_new:server:RemoveInstallation', function(App)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
