...
function closeBanking() {
    $.post("http://ctn-atms/NUIFocusOff", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post("http://ctn-atms/playATMAnim", JSON.stringify({}));
    ...
    $(document).on('click','[data-action=removeCard]',function(){
        var selectedCard = $(this).data('card')
        $.post("http://ctn-atms/removeCard", JSON.stringify({
    ...
    $(document).on('click','[data-action=ATMwithdraw]',function(){
        ...
        if(amount !== undefined && amount !== null && amount !== 0) {
            $.post("http://ctn-atms/doATMWithdraw", JSON.stringify({
    ...
    $(document).on('click','[data-act=enterNumber]',function(){
            ...
            if (number == "ACCEPT") {
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#pinCode').val('');
                    $.post("http://ctn-atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {
