...
QBCore.Functions.CreateCallback('ctn-debitcard:server:requestCards', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('ctn-debitcard:server:deleteCard', function(source, cb, data)
...
RegisterCommand('atm', function(source, args, rawCommand)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            if xCH ~= nil then
                ...
                QBCore.Functions.ExecuteSql(false, "SELECT `charinfo` FROM `players` WHERE `citizenid` = '"..info.citizenid.."'", function(player)
    ...
    TriggerClientEvent('ctn-atms:client:loadATM', src, cards)
end)
...
RegisterServerEvent('ctn-atms:server:doAccountWithdraw')
AddEventHandler('ctn-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then 
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        ...
        if dailyWith < 5000 then
            ...
            if xCH ~= nil then
                ...
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '"..cardHolder.."'", function(player)
            ...
            TriggerClientEvent('ctn-atms:client:updateBankInformation', src, banking)
        else
...
QBCore.Functions.CreateCallback('ctn-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if xCH ~= nil then
        ...
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '"..info.citizenid.."'", function(player)
            local xCH = json.decode(player[1])
            banking['online'] = false
            banking['name'] = xCH.charinfo.firstname .. ' ' .. xCH.charinfo.lastname
            banking['bankbalance'] = xCH.money.bank
