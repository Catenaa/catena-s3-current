$(document).ready(function(){
  ...
  $(".btnPrev").click(function(){
      $.post('http://ctn-contracts/previousID', JSON.stringify({}));
  });

  $(".btnNext").click(function(){
      $.post('http://ctn-contracts/nextID', JSON.stringify({}));
  });

  $(".btnGive").click(function(){
      $.post('http://ctn-contracts/giveID', JSON.stringify({ target: $("#contractID2").val(), conamount: $("#contractAmount2").val(), coninformation: $("#contractInfo2").val()}));
  });

  $(".btnAccept").click(function(){
    $.post('http://ctn-contracts/accept', JSON.stringify({ target: $("#contractID2").val(), conamount: $("#contractAmount2").val(), coninformation: $("#contractInfo2").val()}));
  });

  $(".btnDeny").click(function(){
    $.post('http://ctn-contracts/deny', JSON.stringify({ target: $("#contractID2").val(), conamount: $("#contractAmount2").val(), coninformation: $("#contractInfo2").val()}));
  });
  ...
  document.onkeyup = function (data) {
    if ( data.which == 27 ) {
      $.post('http://ctn-contracts/close', JSON.stringify({}));
    }
    if ( data.which == 37 ) {
      $.post('http://ctn-contracts/previousID', JSON.stringify({}));
    }
