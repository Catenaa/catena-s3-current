...
RegisterNetEvent('ctn-fitbit:use')
AddEventHandler('ctn-fitbit:use', function()
  openWatch(true)
end)
...
RegisterNUICallback('setFoodWarning', function(data)
    ...
    TriggerServerEvent('ctn-fitbit:server:setValue', 'food', foodValue)

    QBCore.Functions.Notify('Fitbit: Food warning set to '..foodValue..'%')
end)

RegisterNUICallback('setThirstWarning', function(data)
    local thirstValue = tonumber(data.value)

    TriggerServerEvent('ctn-fitbit:server:setValue', 'thirst', thirstValue)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            QBCore.Functions.TriggerCallback('ctn-fitbit:server:HasFitbit', function(hasItem)
                if hasItem then
                    ...
                    if PlayerData.metadata["fitbit"].thirst ~= nil then
                        if PlayerData.metadata["thirst"] < PlayerData.metadata["fitbit"].thirst  then
                            TriggerEvent("chatMessage", "FITBIT ", "warning", "Your hydration is "..round(PlayerData.metadata["thirst"], 2).."%")
                            PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
                        end
                    end
                end
