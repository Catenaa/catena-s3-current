...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ctn-admin:server:loadPermissions")
end)
...
RegisterNetEvent('ctn-admin:client:openMenu')
AddEventHandler('ctn-admin:client:openMenu', function(group, dealers)
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('admin', 'Catena Admin')
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('adminOpt') then
            ...
            if WarMenu.Button("Clothing Menu") then
                WarMenu.CloseMenu()
                TriggerServerEvent('ctn-admin:server:OpenSkinMenu', GetPlayerServerId(PlayerId()), 'clothesmenu')
            end

            if WarMenu.Button('Revive') then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent('ctn-admin:server:revivePlayer', targetId)
            end
            ...
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent("ctn-admin:server:togglePlayerNoclip", targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('managementOptions') then
            ...
            if WarMenu.MenuButton('Mechanic Menu', 'managementOptions') then
                WarMenu.CloseMenu()
                TriggerEvent('ctn-mechanic:forceOpen')
            end
            ...
            if WarMenu.ComboBox('Weapon', Weapons, currentWeaponIndex, selectedWeaponIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("ctn-admin:server:spawnWeapon", WeaponsHashs[currentWeaponIndex])
            end

            if WarMenu.MenuButton('Max Ammo', 'managementOptions') then
                TriggerEvent('ctn-weapons:client:SetWeaponAmmoManual', 'current', 250)
            end
        ...
        elseif WarMenu.IsMenuOpened('vehOptions') then
            ...
            if WarMenu.MenuButton('Set Owned', 'vehOptions') then
				...
				TriggerServerEvent('ctn-garages:server:setVehicleOwned', props, {damage = 10, fuel = 10}, model)
            end

            if WarMenu.MenuButton('Fill Fuel', 'vehOptions') then
                local vehicle = (GetVehiclePedIsIn(PlayerPedId()))
                exports['ctn-hud']:SetFuel(vehicle, 99)
            end
        ...
        elseif WarMenu.IsMenuOpened('serverMan') then
            ...
            if WarMenu.ComboBox('Server time', times, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("ctn-weathersync:server:setTime", time.hour, time.minute)
            end
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            if WarMenu.MenuButton('Crash', currentPlayer) then
                TriggerServerEvent('ctn-admin:server:crash', GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Steal Outfit', currentPlayer) then
                TriggerServerEvent("ctn-clothing:stealOutfit", GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Kill', currentPlayer) then
                TriggerServerEvent("ctn-admin:server:killPlayer", GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Revive', currentPlayer) then
                TriggerServerEvent('ctn-admin:server:revivePlayer', GetPlayerServerId(currentPlayer))
            end
            
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                TriggerServerEvent("ctn-admin:server:togglePlayerNoclip", GetPlayerServerId(currentPlayer))
            end
            
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                TriggerServerEvent("ctn-admin:server:Freeze", GetPlayerServerId(currentPlayer), isFreeze)
            end
            ...
            if WarMenu.MenuButton("Give Skin Menu", currentPlayer) then
                TriggerServerEvent('ctn-admin:server:OpenSkinMenu', GetPlayerServerId(currentPlayer), 'clothesmenu')
            end
        ...
        elseif WarMenu.IsMenuOpened('teleportOptions') then
            if WarMenu.MenuButton('Goto', currentPlayer) then
                if in_noclip_mode then
                    turnNoClipOff()
                    TriggerServerEvent('ctn-admin:server:gotoTp', GetPlayerServerId(currentPlayer), GetPlayerServerId(PlayerId()))
            ...
            if WarMenu.MenuButton('Bring', currentPlayer) then
                ...
                TriggerServerEvent('ctn-admin:server:bringTp', GetPlayerServerId(currentPlayer), plyCoords)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent('ctn-admin:server:setPermissions', target, group)
        ...
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban Length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("ctn-admin:server:banPlayer", target, time, reason)
                end
            end
            if WarMenu.MenuButton('Kick', currentPlayer) then
                ...
                if reason ~= nil and reason ~= "" then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("ctn-admin:server:kickPlayer", target, reason)
                end
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    TriggerServerEvent('ctn-weathersync:server:setWeather', AvailableWeatherTypes[k].weather)
...
RegisterNetEvent('ctn-admin:client:bringTp')
AddEventHandler('ctn-admin:client:bringTp', function(coords)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('ctn-admin:client:gotoTp')
AddEventHandler('ctn-admin:client:gotoTp', function(targetId)
    local ped = GetPlayerPed(-1)
    local coords = GetEntityCoords(ped)
    TriggerServerEvent('ctn-admin:server:gotoTpstage2', targetId, coords)
end)

RegisterNetEvent('ctn-admin:client:gotoTp2')
AddEventHandler('ctn-admin:client:gotoTp2', function(coords)
    print(coords)
    local ped = GetPlayerPed(-1)
    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('ctn-admin:client:Freeze')
AddEventHandler('ctn-admin:client:Freeze', function(toggle)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('ctn-admin:client:SendReport')
AddEventHandler('ctn-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('ctn-admin:server:SendReport', name, src, msg)
end)

RegisterNetEvent('ctn-admin:client:SendStaffChat')
AddEventHandler('ctn-admin:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('ctn-admin:server:StaffChatMessage', name, msg)
end)

RegisterNetEvent('ctn-admin:client:SaveCar')
AddEventHandler('ctn-admin:client:SaveCar', function()
    local ped = GetPlayerPed(-1)
    local veh = GetVehiclePedIsIn(ped)
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.VehicleModels[hash] ~= nil and next(QBCore.Shared.VehicleModels[hash]) ~= nil then
            TriggerServerEvent('ctn-admin:server:SaveCar', props, QBCore.Shared.VehicleModels[hash], GetHashKey(veh), plate)
        else
...
RegisterNetEvent('ctn-admin:client:SetModel')
AddEventHandler('ctn-admin:client:SetModel', function(skin)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('ctn-admin:client:SetSpeed')
AddEventHandler('ctn-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('ctn-admin:client:GiveNuiFocus')
AddEventHandler('ctn-admin:client:GiveNuiFocus', function(focus, mouse)
    SetNuiFocus(focus, mouse)
end)

RegisterNetEvent('ctn-admin:client:EnableKeys')
AddEventHandler('ctn-admin:client:EnableKeys', function()
    print('[ctn-admin] Enabled Keys.')
    EnableAllControlActions(0)
    SetNuiFocus(true, true)
end)

RegisterNetEvent('ctn-admin:client:crash')
AddEventHandler('ctn-admin:client:crash', function()
    while true do end
end)
