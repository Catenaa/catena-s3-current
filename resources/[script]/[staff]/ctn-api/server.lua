...
function verify_token(req, res)
    print("[ctn-api] api-request received for: " .. req.path)
    ...
    exports['ghmattimysql']:execute("SELECT * FROM api_tokens WHERE token = @token", {['@token'] = token}, function(result)
        ...
        exports['ghmattimysql']:execute("DELETE FROM api_tokens WHERE token = @token", {['@token'] = data.token}, function() 
			if (data.purpose ~= req.path) then
				print("[ctn-api] api-key purpose failed! Was "..req.path.." but should be "..data.purpose)
				return fail(req, res)
			end
...
function handleRequest(req, res)
    local path = req.path
    print("[ctn-api] executing api call: " .. req.path)
    if path == "kick" then
...
function handleRevive(req, res)
    ...
    TriggerEvent("ctn-admin:server:revivePlayer", playerId)
    res.send(json.encode({success=true}))
    return true
end
...
function handleKickAll(req, res)
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		if v ~= src then 
			DropPlayer(v, "You have been kicked from the server:\n"..reason.."\n\n🔸 Check out our discord for more information: https://discord.gg/Gb3qY9b")
		end
	end
    res.send(json.encode({success=true}))
    return true
