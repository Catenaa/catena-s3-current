...
QBCore.Functions.CreateCallback('ctn-anticheat:server:GetPermissions', function(source, cb)
...
RegisterServerEvent('ctn-anticheat:server:banPlayer')
AddEventHandler('ctn-anticheat:server:banPlayer', function(reason)
    local src = source
    TriggerClientEvent('chatMessage', -1, "CTN Anti-Cheat", "error", GetPlayerName(src).." is banned for: " ..reason )
    ...
    DropPlayer(src, "You have been flagged too many times by our anticheat and prevented from being banned from the server. Check our discord for more information: https://discord.gg/Ttr6fY6")
end)

-- Fake events --
function NonRegisteredEventCalled(CalledEvent, source)
    TriggerClientEvent("ctn-anticheat:client:NonRegisteredEventCalled", source, "Cheating", CalledEvent)
end
...
QBCore.Functions.CreateCallback('ctn-anticheat:server:HasWeaponInInventory', function(source, cb, WeaponInfo)
    local src = source
