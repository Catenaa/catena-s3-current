    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            ...
            if WarMenu.MenuButton('Revive', currentPlayer) then
                ...
                TriggerServerEvent('ctn-admin:server:revivePlayer', target)
            end
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("ctn-admin:server:togglePlayerNoclip", target)
            end
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("ctn-admin:server:Freeze", target, isFreeze)
            end
            ...
            if WarMenu.MenuButton("Give Clothing Menu", currentPlayer) then
                ...
                TriggerServerEvent('ctn-admin:server:OpenSkinMenu', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                QBCore.Functions.Notify(''..GetPlayerName(currentPlayer)..'\'s group has changed to '..group.label)
            end
            WarMenu.Display()
        ...
        elseif WarMenu.IsMenuOpened('dealerManagement') then
            ...
            WarMenu.MenuButton('Place Dealer', 'createDealer')
        ...
        elseif WarMenu.IsMenuOpened('createDealer') then
            ...
            if WarMenu.MenuButton("Create Dealer", 'createDealer') then
...
RegisterNetEvent('ctn-weapons:client:SetWeaponAmmoManual')
AddEventHandler('ctn-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = GetPlayerPed(-1)
    if weapon ~= "current" then
        ...
        if weapon ~= nil then
            ...
            QBCore.Functions.Notify('You don\'t have a weapon..', 'error')
        end
    end
end)
