$(function(){
	...
	$('.users').on('click', '.spec', function(){
		...
		if (target == player) {
			...
			$('.spectate').fadeOut();
			$.post('http://ctn-spectate/select', JSON.stringify({id: target}));
		}
	...
	$('.users').on('click', '.kick', function(){
		...
		if (reason == null || reason == "") {
		    ...
		    $('.spectate').fadeOut();
			$.post('http://ctn-spectate/kick', JSON.stringify({id: target, reason: reason}));
		}
	});

	$('.header').on('click', '#close', function(){
		$('.spectate').fadeOut();
		$.post('http://ctn-spectate/quit');
	});
