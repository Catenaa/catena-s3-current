...
Config.Language = {
    ...
    idrr = "\xE2\x9D\x97[Queue] Error: Can't retrieve IDs, try restarting.",
    ...
    wlonly = "\xE2\x9D\x97[Queue] You have to have a whitelist to join the server..",
    steam = "\xE2\x9D\x97 [Queue] Error: Steam should be on.."
}
...
QBCore.Commands.Add("reloadqueuepriority", "Reload queue priority", {{name="id", help="Player ID"}, {name="priority", help="Priority level"}}, true, function(source, args)
	loadDatabaseQueue()
	TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "REFRESH")	
end, "god")

QBCore.Commands.Add("addpriority", "Add queue priority", {{name="id", help="Player ID"}, {name="priority", help="Priority level"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
	local level = tonumber(args[2])
	if Player ~= nil then
        AddPriority(Player.PlayerData.source, level)
        TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "You've got " .. GetPlayerName(Player.PlayerData.source) .. " prioritised ("..level..")")	
	else
...
QBCore.Commands.Add("removepriority", "Take priority away from someone", {{name="id", help="Player ID"}}, true, function(source, args)
	...
	if Player ~= nil then
        ...
        TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "You've taken priority away from " .. GetPlayerName(Player.PlayerData.source))	
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Player is not online!")	
	end
end, "god")
