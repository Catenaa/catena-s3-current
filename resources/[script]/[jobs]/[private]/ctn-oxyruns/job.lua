...
CreateThread(function()
    Wait(500)
    TriggerServerEvent('ctn-oxyruns:server:doorState')
    ...
    while true do
        ...
        local distance = {}
        local whitelisted = (QBCore and QBCore.Functions.GetPlayerData().job and QBCore.Functions.GetPlayerData().job.name == 'drugdealer')
        local isboss = (QBCore and QBCore.Functions.GetPlayerData().job and QBCore.Functions.GetPlayerData().job.isboss)
        ...
        if (distance['enter'] < 5) then
            if (whitelisted) then
                ...
                if (IsControlJustPressed(0, 74)) then 
                    openDoorAnim()
                    TriggerServerEvent('ctn-oxyruns:server:doorState', not DoorLocked)
                end
            ...
            if IsControlJustPressed(0, 38) and (whitelisted or not DoorLocked) then
                openDoorAnim()
                Wait(500)
                Objects, POIOffsets = exports['ctn-interior']:CreateGunshopShell({ x = plyCoords.x, y = plyCoords.y, z = plyCoords.z-50.0, h = GetEntityHeading(plyPed) })
                CreateDrugStorePed()
            end
...
RegisterNetEvent('ctn-oxyruns:client:doorState')
AddEventHandler('ctn-oxyruns:client:doorState', function(bool)
    DoorLocked = bool
end)
...
RegisterNetEvent("ctn-oxyruns:client:AcceptBribe")
AddEventHandler("ctn-oxyruns:client:AcceptBribe", function()
    openDoorAnim()
    PlayAmbientSpeech1(drugStorePed, "Chat_Resp", "SPEECH_PARAMS_FORCE", 1)
    openDoorAnim(drugStorePed)
end)

RegisterNetEvent("ctn-oxyruns:client:nope")
AddEventHandler("ctn-oxyruns:client:nope", function()
    QBCore.Functions.Notify('We aint needing no help right now - come back later.', "error")
	PlayAmbientSpeech1(drugStorePed, "Chat_Resp", "SPEECH_PARAMS_FORCE", 1)
end)
...
function RefreshBlip()
    ...
    if (QBCore and QBCore.Functions.GetPlayerData().job and QBCore.Functions.GetPlayerData().job.name == 'drugdealer') then
