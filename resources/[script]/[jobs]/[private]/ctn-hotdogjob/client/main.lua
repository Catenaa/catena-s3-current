...
function StartWorking()
    QBCore.Functions.TriggerCallback('ctn-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = GetPlayerPed(-1)
...
RegisterNetEvent('ctn-hotdogjob:client:UpdateReputation')
AddEventHandler('ctn-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('ctn-hotdogjob:client:ToggleSell')
AddEventHandler('ctn-hotdogjob:client:ToggleSell', function(data)
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x for €'..(HotdogsForSale * SellingPrice)..',- / [8] Reject')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    ...
                    TriggerServerEvent('ctn-hotdogjob:server:Sell', HotdogsForSale, SellingPrice)
                    SellingData.HasTarget = false
...
function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('ctn-keyminigame:show')
    TriggerEvent('ctn-keyminigame:start', FinishMinigame)
end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('ctn-hotdogjob:server:UpdateReputation', Quality)
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('ctn-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
...
RegisterNetEvent('ctn-hotdogjob:staff:DeletStand')
AddEventHandler('ctn-hotdogjob:staff:DeletStand', function()
    local ped = GetPlayerPed(-1)
