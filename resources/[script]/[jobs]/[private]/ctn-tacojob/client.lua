...
RegisterNetEvent("ctn-taco:client:main")
AddEventHandler("ctn-taco:client:main", function(job)
	if taskingRun then
		return
	end
	...
	while true do
		...
		if job ~= nil then
			...
			if dst_cooking < 1.5 and not isInRun then
				...
				if not IsPedInAnyVehicle(playerPed) and IsControlJustPressed(0, 38) then
					...
					for counter = 1, skillbarAmount do
						local finished = exports["ctn-taskbarskill"]:taskBar(math.random(1600, 2500), math.random(5, 15))
					...
					if done == true then
						TriggerServerEvent('ctn-taco:server:addItem', job['settings'].food_item, 2)
			...
			if dst_packaging < 1.5 and not isInRun then
				...
				if not IsPedInAnyVehicle(playerPed) and IsControlJustPressed(0, 38) then
					Wait(10)
					QBCore.Functions.TriggerCallback('ctn-taco:server:getItemCount', function(hasItems)
						if hasItems then
							playerAnim(2)

							local skillbarAmount, done = math.random(1, 4), true
							for counter = 1, skillbarAmount do
								local finished = exports["ctn-taskbarskill"]:taskBar(math.random(2000, 2500), math.random(5, 15))
								if finished ~= 100 then
        	    					done = false
								end
							end
							...
							ClearPedTasks(playerPed)
							if done == true then
								TriggerServerEvent('ctn-taco:server:removeItem', job['settings'].food_item, 2)
								TriggerServerEvent('ctn-taco:server:addItem', job['settings'].bag_item, 1)
			...
			if dst_ready < 1.5 and not isInRun then
				...
				if not IsPedInAnyVehicle(playerPed) and IsControlJustPressed(0, 38) then
					...
					QBCore.Functions.Progressbar("rljobs_preparing", job['settings'].preparing, 3000, false, true, {
						...
						TriggerServerEvent('ctn-taco:server:readyForDelivery', job['name'], job['settings'].bag_item)
					end, function()
		...
		for _, job in pairs(Config.Jobs) do
			if not job['settings'].private_deliveries then
				...
				if dst_pickup < 1.5 and not isInRun then
					...
					if not IsPedInAnyVehicle(playerPed) and IsControlJustPressed(0, 38) then
                        Wait(10)
						TriggerServerEvent('ctn-taco:server:startFromDelivery', job['name'])
					end
				end
				...
				if dst_cancle < 1.5 and isInRun then
					...
					if not IsPedInAnyVehicle(playerPed) and IsControlJustReleased(0,38) then
						Wait(10)
						TriggerServerEvent('ctn-taco:server:removeItem', job['settings'].bag_item, 1)
	...
	while true do
        ...
        if isInRun and runJob then
			if taskingRun then
				...
				TriggerEvent("ctn-taco:client:main", runJob)
			end
		end
		Wait(0)
    end
...
RegisterNetEvent("ctn-taco:client:startDelivery")
AddEventHandler("ctn-taco:client:startDelivery", function(job)
	isInRun = true
	runJob = job
end)
...
function DoDropOff(Location)
	...
	Citizen.Wait(2000)
	TriggerServerEvent("ctn-taco:server:dropoff", runJob)
