        ...
        Citizen.CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    QBCore.Functions.Notify("You've bled out..", "error")
                    SetLaststand(false)
                    ...
                    TriggerServerEvent("ctn-log:server:CreateLog", "death", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") is dead", "red", "**".. killerName .. "** has ".. GetPlayerName(player) .." killed by **".. weaponLabel .. "** (" .. weaponName .. ")")
                    deathTime = 0
                    OnDeath()
                    DeathTimer()
                end
...
AddEventHandler('hospital:client:HelpPerson', function(targetId)
    ...
    QBCore.Functions.Progressbar("hospital_revive", "Help person up..", math.random(30000, 60000), false, true, {
        ...
        ClearPedTasks(ped)
        QBCore.Functions.Notify("You helped the person up.")
