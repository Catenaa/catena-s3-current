...
RegisterNetEvent("ctn-carry:beingCarried")
AddEventHandler("ctn-carry:beingCarried", function(bool)
    inCarry = bool
end)
        ...
        if QBCore ~= nil then
            ...
            if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, true) < 1.5) then
                if doctorCount >= Config.MinimalDoctors then
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "[E] - Call a doctor")
                else
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "[E] - Check In")
                end
                if IsControlJustReleased(0, Keys["E"]) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        QBCore.Functions.Progressbar("hospital_checkin", "Checking In", 6000, false, true, {
                            ...
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            QBCore.Functions.Notify("Not checked in!", "error")
                        end)
                    end
                end
            elseif (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, true) < 4.5) then
                if doctorCount >= Config.MinimalDoctors then
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Call a doctor")
                else
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Check In")
                end
            end

            if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["pharmacy"].x, Config.Locations["pharmacy"].y, Config.Locations["pharmacy"].z, true) < 1.5) then
                QBCore.Functions.DrawText3D(Config.Locations["pharmacy"].x, Config.Locations["pharmacy"].y, Config.Locations["pharmacy"].z, "[E] - Pharmacy")
                if IsControlJustReleased(0, Keys["E"]) then
                    TriggerServerEvent("inventory:server:OpenInventory", "shop", "pharmacy", Config.PharmacyItems)
                end
            elseif (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["pharmacy"].x, Config.Locations["pharmacy"].y, Config.Locations["pharmacy"].z, true) < 4.5) then
                QBCore.Functions.DrawText3D(Config.Locations["pharmacy"].x, Config.Locations["pharmacy"].y, Config.Locations["pharmacy"].z, "Pharmacy")
            end
            ...
            if closestBed ~= nil and not isInHospitalBed then
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z, true) < 1.5) then
                    QBCore.Functions.DrawText3D(Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z + 0.3, "~g~E~w~ - To lay in bed")
                    if IsControlJustReleased(0, Keys["E"]) then
...
AddEventHandler('hospital:client:Revive', function()
    ...
    TriggerServerEvent('ctn-hud:Server:RelieveStress', 100)
    ...
    QBCore.Functions.Notify("You are completely top again!")
end)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        ...
        local player = PlayerPedId()
        if isRevive then
            QBCore.Functions.Notify("You will be helped..")
...
AddEventHandler('hospital:removeMoney', function(source)
    local src = source
    local source = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveMoney('cash', 250, 'Hospital removeal')
end)
