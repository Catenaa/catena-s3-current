...
AddEventHandler("mdt:performVehicleSearch", function(query)
	local src = source
	local matches = {}
	exports['ghmattimysql']:execute("SELECT * FROM `ctnvehicles` WHERE LOWER(`plate`) LIKE @query", {
		['@query'] = string.lower('%'..query..'%') -- % wildcard, needed to search for all alike results
	}, function(result)

		for index, data in ipairs(result) do
			local data_decoded = json.decode(data.props)
...
AddEventHandler("mdt:performVehicleSearchInFront", function(query)
	...
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
		exports['ghmattimysql']:execute("SELECT * FROM `ctnvehicles` WHERE `plate` = @query", {
			['@query'] = query
		}, function(result)
