...
QBCore.Commands.Add('unjail', 'Unjail player (Police Only)', {{name = 'ID', help = 'Player ID'}}, true, function(source, args)
    ...
    if xPlayer.PlayerData.job.name == 'police' then
        if tonumber(args[1]) and QBCore.Functions.GetPlayer(tonumber(args[1])) then
...
QBCore.Commands.Add('jail', 'Jail player (Police Only)', {{name = 'ID', help = 'Player ID'}, {name = 'Time', help = 'Time'}, {name = 'Reason', help = 'reason'}}, true, function(source, args)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    if xPlayer.PlayerData.job.name == 'police' then
        if tonumber(args[1]) and QBCore.Functions.GetPlayer(tonumber(args[1])) then
