...
AddEventHandler('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if CuffedPlayer ~= nil then
        if Player.Functions.GetItemByName("handcuffs") ~= nil or Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
            TriggerClientEvent("police:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)           
        else
            TriggerClientEvent('QBCore:Notify', src, "You dont have handcuffs and you are not officer.", "error")
        end
    end
end)
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    ...
    if EscortPlayer ~= nil then
        if (Player.PlayerData.job.name == "gendarme" or Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
QBCore.Commands.Add("fine", "Bill Player", {{name="id", help="Player ID"}, {name="amount", help="Fine Amount"}, {name="title", help="Fine Title"}}, false, function(source, args)
    ...
    if Player.PlayerData.job.name == "gendarme" or Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == 'ambulance' or Player.PlayerData.job.name == 'mechanic' or Player.PlayerData.job.name == 'cardealer' then
        if OtherPlayer ~= nil then
            if price and price > 0 then
                QBCore.Functions.ExecuteSql(false, "INSERT INTO `phone_invoices` (`citizenid`, `amount`, `society`, `title`) VALUES ('" .. OtherPlayer.PlayerData.citizenid .. "', " .. price .. ", '" .. Player.PlayerData.job.name .. "', '" .. title .. "')", function()
                    ...
                    TriggerClientEvent("ctn-phone:RefreshPhone", OtherPlayer.PlayerData.source)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
        if OtherPlayer ~= nil then
...
AddEventHandler('police:server:FlaggedPlateTriggered', function(camId, plate, street1, street2, blipSettings)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme") and Player.PlayerData.job.onduty then
                if street2 ~= nil then
...
AddEventHandler('police:server:VehicleCall', function(pos, msg, alertTitle, streetLabel, modelPlate, modelName)
    ...
    TriggerClientEvent("ctn-phone:client:addPoliceAlert", -1, alertData)
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Can't take driving license..", "error")
        end
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            exports['ghmattimysql']:execute('UPDATE ctnvehicles SET state = @state, depotprice = @depotprice WHERE plate = @plate', {['@state'] = 0, ['@depotprice'] = price, ['@plate'] = plate})
            TriggerClientEvent('QBCore:Notify', src, "Vehicle included in depot for €"..price.."!")
        else
            exports['ghmattimysql']:execute('UPDATE ctnvehicles SET state = @state WHERE plate = @plate', {['@state'] = 2, ['@plate'] = plate})
...
AddEventHandler('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('QBCore:Notify', targetId, 'You got an ankle bracelet.', 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("ctn-phone:client:addPoliceAlert", requestId, alertData)
end)
...
QBCore.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then 
            if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "gendarme") and Player.PlayerData.job.onduty) then
                table.insert(dutyPlayers, {
                    source = Player.PlayerData.source,
                    label = Player.PlayerData.metadata["callsign"],
                    job = Player.PlayerData.job.name,
...
QBCore.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
    exports['ghmattimysql']:execute('SELECT * FROM ctnvehicles WHERE state = @state', {['@state'] = 2}, function(result)
        if result[1] ~= nil then
            vehicles = result
        end
        cb(vehicles)
...
QBCore.Commands.Add("spike", "Place down a spike strip", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme") and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("pobj", "Place / Delete an object", {{name="type", help="Type object you want or 'delete' to delete"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("cuff", "Cuff citizen", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
...
QBCore.Commands.Add("seatvehicle", "Put player in vehicle", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
...
QBCore.Commands.Add("unseatvehicle", "Get player out from vehicle", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
...
QBCore.Commands.Add("escort", "Escort citizen", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent("police:client:EscortPlayer", source)
end)

QBCore.Commands.Add("clearcasings", "Take away bullets in the area (make sure you have picked up some)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
...
QBCore.Commands.Add("softcuff", "Handcuff someone but let him walk", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
...
QBCore.Commands.Add("camera", "View security camera", {{name="camid", help="Camera ID"}}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
...
QBCore.Commands.Add("dvp", "Send a vehicle to the impound", {{name="price", help="Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" then
...
--QBCore.Commands.Add("fullimpound", "Seize a vehicle", {}, false, function(source, args)
--	local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("deadofficer", "Send a message back to a notification", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") and Player.PlayerData.job.onduty) then
        TriggerClientEvent("police:client:SendPoliceEmergencyAlert", source)
    end
end)

QBCore.Commands.Add("tds", "Confiscate driver license", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme") and Player.PlayerData.job.onduty) then
...
QBCore.Commands.Add("tdna", "Take a DNA copy from a person (empty proof bag required)", {{"id", "Person ID"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local OtherPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "gendarme") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            local info = {
                label = "DNA-Monster",
                type = "dna",
            ...
            TriggerClientEvent('QBCore:Notify', source, "You must have an empty proof bag with you", "error")
        end
    end
end)
...
QBCore.Commands.Add('sl', 'Use shit lockpick', {}, false, function(source, args)
