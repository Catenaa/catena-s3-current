...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        if isLoggedIn then
            if PlayerJob.name == "police" or PlayerJob.name == "gendarme" then
                    ...
                    if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 5) then
                        if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 2) then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                ...
                                TriggerEvent('ctn-policealerts:ToggleDuty', onDuty)
                            end
                        end
                    end
                end
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                        ...
                                        if not IsModelValid(Config.Helicopter) then
                                            QBCore.Functions.Notify("Invaild Vehicle Model", "error")
                                            return false
                                        end

                                        QBCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                                            SetVehicleNumberPlateText(veh, "CCPD"..tostring(math.random(1000, 9999)))
                ...
                for k, v in pairs(Config.Locations["armory"]) do
                    if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 4.5) and IsArmoryWhitelist() then
                        if onDuty then
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Weapons safe")
                                SetWeaponSeries()
                                if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            elseif (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 2.5) then
                                DrawText3D(v.x, v.y, v.z, "Weapons safe")
                            end  
                        end
                    end
                end
...
AddEventHandler('police:client:CheckStatus', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" or PlayerData.job.name == "gendarme" then
            local player, distance = GetClosestPlayer()
...
function TakeOutImpound(vehicle)
    ...
    QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
        QBCore.Functions.TriggerCallback('ctn-garage:server:GetVehicleProperties', function(properties)
