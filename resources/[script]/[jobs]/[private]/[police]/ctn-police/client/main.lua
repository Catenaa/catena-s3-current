...
Citizen.CreateThread(function()
    for k, station in pairs(Config.Locations["stations"]) do
        ...
        if station.label == 'Gendarmerie' then
            SetBlipColour(blip, 2)
            SetBlipScale(blip, 1.0)
        elseif station.label == 'Prison' then
...
AddEventHandler('QBCore:Client:OnJobUpdate', function(JobInfo)
    ...
    if (JobInfo.name == "police" and not PlayerJob.onduty) or (JobInfo.name == "gendarme" and not PlayerJob.onduty) then
        TriggerServerEvent("QBCore:ToggleDuty")
        onDuty = true
    end

    if (PlayerJob ~= nil) and (PlayerJob.name ~= "police" or PlayerJob.name ~= "gendarme") then
...
Citizen.CreateThread(function()
    ...
    if QBCore.Functions.GetPlayerData().metadata["tracker"] then
        ...
        TriggerEvent('ctn-clothings:client:loadOutfit', trackerClothingData)
    else
...
AddEventHandler('police:client:SendPoliceEmergencyAlert', function()
    ...
    TriggerServerEvent('ctn-policealerts:server:AddPoliceAlert', {
        timeOut = 10000,
...
AddEventHandler('police:client:VehicleCall', function(pos, alertTitle, streetLabel, modelPlate, modelName)
    if PlayerJob.name == 'police' and onDuty then
        TriggerEvent('ctn-policealerts:client:AddPoliceAlert', {
            timeOut = 4000,
            alertTitle = alertTitle,
            coords = {
                x = pos.x,
...
AddEventHandler('police:server:SendEmergencyMessageCheck', function(MainPlayer, message, coords)
    ...
    if ((PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "doctor" or PlayerData.job.name == "gendarme") and onDuty) then
...
AddEventHandler('police:client:Send112AMessage', function(message)
    local PlayerData = QBCore.Functions.GetPlayerData()
    if ((PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "gendarme") and onDuty) then
