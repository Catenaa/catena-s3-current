...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            QBCore.Functions.Progressbar("robbing_player", "Robbing", 3500, false, true, {
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(GetPlayerPed(-1))) then
                ...
                QBCore.Functions.Notify("You cannot soft cuff in a vehicle", "error")
            end
        else
            QBCore.Functions.Notify("No one around!", "error")
        end
...
AddEventHandler('police:client:GetKidnappedDragger', function(playerId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        TriggerEvent('ctn-kidnapping:client:SetKidnapping', isEscorting)
    end)
end)
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    ...
    local finished = 0
    if not QBCore.Functions.GetPlayerData().metadata["isdead"] and lastSkill < 3 and not isHandcuffed then
        finished = exports["ctn-taskbarskill"]:taskBar(1200,7)
    ...
    if finished == 100 then return end
        ...
        if not isSoftcuff then
            ...
            QBCore.Functions.Notify("You are fascinated!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            QBCore.Functions.Notify("You are fascinated, but you can walk")
        end
        ...
        ClearPedTasksImmediately(GetPlayerPed(-1))
        QBCore.Functions.Notify("You are untied!")
    end
end)
...
AddEventHandler('police:client:UseShitLockpick', function(item)
    if isHandcuffed then
        TriggerServerEvent('QBCore:Server:RemoveItem', 'shitlockpick', 1)
        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["shitlockpick"], "remove")
        Wait(1200)
        if exports["ctn-taskbarskill"]:taskBar(2500,math.random(5,20)) then
