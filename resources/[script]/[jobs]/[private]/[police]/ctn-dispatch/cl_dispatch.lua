    ...
    if pNotificationData ~= nil and QBCore and QBCore.Functions.GetPlayerData().job then
        if pNotificationData.recipientList then
            for key, value in pairs(pNotificationData.recipientList) do
                if key == currentJob and value and not disableNotifications and QBCore.Functions.GetPlayerData().job.onduty then
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 57) and (currentJob == "police" or currentJob == "ambulance" or currentJob == "reporter") and not QBCore.Functions.GetPlayerData().metadata["isdead"]) then
