...
RegisterNetEvent("ctn-houses:client:decorate")
AddEventHandler("ctn-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
...
RegisterNetEvent("ctn-houses:server:sethousedecorations")
AddEventHandler("ctn-houses:server:sethousedecorations", function(house, decorations)
...
function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('ctn-anticheat:client:ToggleDecorate', true)
end
...
function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('ctn-anticheat:client:ToggleDecorate', false)
end
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		QBCore.Functions.TriggerCallback('ctn-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("ctn-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
