...
Citizen.CreateThread(function() 
    ...
    TriggerServerEvent('ctn-houses:server:init')
end)

RegisterNetEvent('ctn-houses:client:sellHouse')
AddEventHandler('ctn-houses:client:sellHouse', function()
    if closesthouse ~= nil and hasKey then
        TriggerServerEvent('ctn-houses:server:viewHouse', closesthouse)
    end
end)
...
RegisterNetEvent('ctn-houses:client:EnterHouse')
AddEventHandler('ctn-houses:client:EnterHouse', function(god)
...
RegisterNetEvent('ctn-houses:client:RequestRing')
AddEventHandler('ctn-houses:client:RequestRing', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)

    if closesthouse ~= nil then
        TriggerServerEvent('ctn-houses:server:RingDoor', closesthouse)
    end
end)
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('ctn-houses:client:setHouses')
    isLoggedIn = true
    SetClosestHouse()
    TriggerEvent('ctn-houses:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerEvent('ctn-garages:client:setHouseGarage', closesthouse, hasKey)
    TriggerServerEvent("ctn-houses:server:setHouses")
end)
...
RegisterNetEvent('ctn-houses:client:setHouseConfig')
AddEventHandler('ctn-houses:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
end)

RegisterNetEvent('ctn-houses:client:lockHouse')
AddEventHandler('ctn-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('ctn-houses:client:createHouses')
AddEventHandler('ctn-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('ctn-houses:server:addNewHouse', street, coords, price, tier)
end)

RegisterNetEvent('ctn-houses:client:addGarage')
AddEventHandler('ctn-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        local pos = GetEntityCoords(GetPlayerPed(-1))
        local heading = GetEntityHeading(GetPlayerPed(-1))
        local coords = {
        ...
        TriggerServerEvent('ctn-garages:server:addHouseGarage', closesthouse, coords, max)
    else
        QBCore.Functions.Notify("No house nearby.", "error")
    end
end)

RegisterNetEvent('ctn-houses:client:toggleDoorlock')
AddEventHandler('ctn-houses:client:toggleDoorlock', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('ctn-houses:server:lockHouse', false, closesthouse)
                QBCore.Functions.Notify("House is unlocked!", "success", 2500)
            else
                TriggerServerEvent('ctn-houses:server:lockHouse', true, closesthouse)
        ...
        QBCore.Functions.Notify("There is no door to be seen?", "error", 3500)
    end
end)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                TriggerServerEvent("ctn-houses:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
                    ...
                    if CurrentHouse ~= nil then
                        if outfitLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, outfitLocation.x, outfitLocation.y, outfitLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent("ctn-clothing:client:openOutfitMenu", true)
                                end
                        ...
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    exports['ctn-interior']:DespawnInterior(houseObj, function()
                                        TriggerEvent('ctn-weathersync:client:EnableSync')
                                        ...
                                        inOwned = false
                                        inside = false
                                        TriggerServerEvent('ctn-houses:server:LogoutLocation')
                                    end)
                                end
...
RegisterNetEvent('ctn-houses:client:RingDoor')
AddEventHandler('ctn-houses:client:RingDoor', function(player, house)
...
RegisterNetEvent('ctn-houses:client:giveHouseKey')
AddEventHandler('ctn-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('ctn-houses:server:giveHouseKey', playerId, closesthouse)
        else
...
RegisterNetEvent('ctn-houses:client:removeHouseKey')
AddEventHandler('ctn-houses:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            QBCore.Functions.TriggerCallback('ctn-houses:server:getHouseOwner', function(result)
                if QBCore.Functions.GetPlayerData().citizenid == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
                    Menu.hidden = not Menu.hidden
                else
                    QBCore.Functions.Notify("You are not a home owner..", "error")
                end
            end, closesthouse)
        else
            QBCore.Functions.Notify("You are not close enough to the house..", "error")
...
RegisterNetEvent('ctn-houses:client:refreshHouse')
AddEventHandler('ctn-houses:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
    --TriggerEvent('ctn-garages:client:setHouseGarage', closesthouse, hasKey)
end)

RegisterNetEvent('ctn-houses:client:SpawnInApartment')
AddEventHandler('ctn-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(GetPlayerPed(-1))
...
function HouseKeysMenu()
    ...
    MenuTitle = "Keys"
    ClearMenu()
    QBCore.Functions.TriggerCallback('ctn-houses:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
...
function removeHouseKey(citizenData)
    TriggerServerEvent('ctn-houses:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['ctn-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 2 then
        data = exports['ctn-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 3 then
        data = exports['ctn-interior']:CreateMichaelShell(coords)
    elseif Config.Houses[house].tier == 4 then
        data = exports['ctn-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 5 then
        data = exports['ctn-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 6 then
        data = exports['ctn-interior']:CreateFranklinShell(coords)
    elseif Config.Houses[house].tier == 7 then
        data = exports['ctn-interior']:CreateFranklinAuntShell(coords)
    elseif Config.Houses[house].tier == 8 then
        data = exports['ctn-interior']:CreateGarageboxShell(coords)
    end
    ...
    entering = true
    TriggerServerEvent('ctn-houses:server:SetInsideMeta', house, true)
    Citizen.Wait(500)
    SetRainFxIntensity(0.0)
    TriggerEvent('ctn-weathersync:client:DisableSync')
    TriggerEvent('ctn-weed:client:getHousePlants', house)
    Citizen.Wait(100)
...
RegisterNetEvent('ctn-houses:client:enterOwnedHouse')
AddEventHandler('ctn-houses:client:enterOwnedHouse', function(house)
...
RegisterNUICallback('HasEnoughMoney', function(data, cb)
    QBCore.Functions.TriggerCallback('ctn-houses:server:HasEnoughMoney', function(hasEnough)
        
    end, data.objectData)
end)

RegisterNetEvent('ctn-houses:client:LastLocationHouse')
AddEventHandler('ctn-houses:client:LastLocationHouse', function(houseId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
function leaveOwnedHouse(house)
    if not FrontCam then
        ...
        exports['ctn-interior']:DespawnInterior(houseObj, function()
            ...
            SetEntityHeading(GetPlayerPed(-1), Config.Houses[CurrentHouse].coords.enter.h)
            TriggerEvent('ctn-weed:client:leaveHouse')
            TriggerServerEvent('ctn-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
        end)
    end
end
...
RegisterNetEvent('ctn-houses:client:setupHouseBlips')
AddEventHandler('ctn-houses:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            QBCore.Functions.TriggerCallback('ctn-houses:server:getOwnedHouses', function(ownedHouses)
...
RegisterNetEvent('ctn-houses:client:SetClosestHouse')
AddEventHandler('ctn-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('ctn-houses:server:buyHouse', closesthouse)
end)
...
RegisterNetEvent('ctn-houses:client:viewHouse')
AddEventHandler('ctn-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            QBCore.Functions.TriggerCallback('ctn-houses:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            QBCore.Functions.TriggerCallback('ctn-houses:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        QBCore.Functions.TriggerCallback('ctn-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
RegisterNetEvent('ctn-houses:client:setLocation')
AddEventHandler('ctn-houses:client:setLocation', function(data)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data == 'setstash' then
                TriggerServerEvent('ctn-houses:server:setLocation', coords, closesthouse, 1)
            elseif data == 'setoutift' then
                TriggerServerEvent('ctn-houses:server:setLocation', coords, closesthouse, 2)
            elseif data == 'setlogout' then
                TriggerServerEvent('ctn-houses:server:setLocation', coords, closesthouse, 3)
            end
        else
            QBCore.Functions.Notify('You do not own the house..', 'error')
        end
    else    
...
RegisterNetEvent('ctn-houses:client:refreshLocations')
AddEventHandler('ctn-houses:client:refreshLocations', function(house, location, type)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('ctn-houses:client:HomeInvasion')
AddEventHandler('ctn-houses:client:HomeInvasion', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local Skillbar = exports['ctn-skillbar']:GetSkillbarObject()
    ...
    if closesthouse ~= nil then
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    ...
                                    TriggerServerEvent('ctn-houses:server:SetHouseRammed', true, closesthouse)
                                ...
                                RamsDone = 0
                                TriggerServerEvent('ctn-houses:server:SetRamState', false, closesthouse)
                                QBCore.Functions.Notify('It failed .. Please try again.', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('ctn-houses:server:SetRamState', true, closesthouse)
                        else
                ...
                QBCore.Functions.Notify('There is no force management present.', 'error')
            end
        end)

RegisterNetEvent('ctn-houses:client:SetRamState')
AddEventHandler('ctn-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
end)

RegisterNetEvent('ctn-houses:client:SetHouseRammed')
AddEventHandler('ctn-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
end)

RegisterNetEvent('ctn-houses:client:ResetHouse')
AddEventHandler('ctn-houses:client:ResetHouse', function()
    local ped = GetPlayerPed(-1)

    if closesthouse ~= nil then
        if Config.Houses[closesthouse].IsRammed == nil then
            Config.Houses[closesthouse].IsRammed = false
            TriggerServerEvent('ctn-houses:server:SetHouseRammed', false, closesthouse)
            TriggerServerEvent('ctn-houses:server:SetRamState', false, closesthouse)
        end
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('ctn-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            QBCore.Functions.Notify('You locked up the house again..', 'success')
        else
            QBCore.Functions.Notify('This door has not been broken open..', 'error')
        end
    end
end)