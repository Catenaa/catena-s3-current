...
CreateThread(function()
    Wait(500)
    TriggerServerEvent('ctn-houses:server:doorState')
    while true do
        ...
        local distance = {}
        local whitelisted = (QBCore and QBCore.Functions.GetPlayerData().job and QBCore.Functions.GetPlayerData().job.name == 'realestate')
        ...
        if (distance['enter'] < 5) then
            if (whitelisted) then
                ...
                if (IsControlJustPressed(0, 74)) then 
                    TriggerServerEvent('ctn-houses:server:doorState', not DoorLocked)
                end
            else
...
RegisterNetEvent('ctn-houses:client:doorState')
AddEventHandler('ctn-houses:client:doorState', function(bool)
    DoorLocked = bool
end)
