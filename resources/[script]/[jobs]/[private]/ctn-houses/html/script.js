...
$('document').ready(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post('http://ctn-houses/toggleCursor');
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null) {
                $.post('http://ctn-houses/editOwnedObject', JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post('http://ctn-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                ...
                if (objData != null) {
                    $.post("http://ctn-houses/spawnobject", JSON.stringify({
...
$(document).on('click', '.header-btn', function(){
    ...
    $.post('http://ctn-houses/ResetSelectedProp');
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post('http://ctn-houses/deleteSelectedObject');
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post('http://ctn-houses/buySelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});

$(document).on('click', '#cancel-object', function(){
    $.post('http://ctn-houses/cancelSelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post('http://ctn-houses/removeObject');
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post("http://ctn-houses/chooseobject", JSON.stringify({
                object: objData.object,
            }));
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post('http://ctn-houses/deselectOwnedObject')
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post('http://ctn-houses/selectOwnedObject', JSON.stringify({
...
Decorations.Close = function() {
    ...
    selectedObjectData = null;
    $.post("http://ctn-houses/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function(e){
    $.post('http://ctn-houses/buy', JSON.stringify({}))
});

$(".property-cancel").click(function(e){
    $.post('http://ctn-houses/exit', JSON.stringify({}));
});