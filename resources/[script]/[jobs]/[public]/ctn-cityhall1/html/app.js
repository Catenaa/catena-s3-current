...
$('.cityhall-option-block').click(function(e){
    ...
    if (blockPage == "identity") {
        ...
        $.post('http://ctn-cityhall/requestLicenses', JSON.stringify({}), function(licenses){
...
hoverDescription = function(type) {
    if (!mouseOver) {
        if (type == "id-card") {
            ...
            $(".hover-description").fadeIn(10);
            $(".hover-description").html('<p>If you drive a vehicle you are required to have a drivers license<br> to be able to show it when it is requested.</p>');
        }
    } else {
...
$(document).on("click", ".identity-page-block", function(e){
    ...
    if (selectedIdentity == null) {
        ...
        if (idType== "id-card") {
            ...
            $(".request-identity-button").html("<p>Apply for a  ID-Card (€50,-)</p>")
        } else {
            $(".request-identity-button").fadeIn(100);
            $(".request-identity-button").html("<p>for a driving license (€50,-)</p>")
        }
...
$(".request-identity-button").click(function(e){
    ...
    $.post('http://ctn-cityhall/requestId', JSON.stringify({
...
$(document).on('click', '.apply-job-button', function(e){
    ...
    $.post('http://ctn-cityhall/applyJob', JSON.stringify({
        job: selectedJobId
    }))
