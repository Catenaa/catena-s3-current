    ...
    while true do
        ...
        if dist < 20 then
            ...
            if GetDistanceBetweenCoords(pos, Config.Cityhall.coords.x, Config.Cityhall.coords.y, Config.Cityhall.coords.z, true) < 1.5 then
                qbCityhall.DrawText3Ds(Config.Cityhall.coords, '~g~E~w~ - Government open')
...
RegisterNUICallback('requestId', function(data)
    if inRange then
        ...
        QBCore.Functions.Notify('U got your '..idTypes[idType].label..' for €50', 'success', 3500)
    else
        QBCore.Functions.Notify('It wont be possible...', 'error')
    end
end)
...
RegisterNUICallback('applyJob', function(data)
    if inRange then
        if IsAvailableJob(data.job) then
            ...
            TriggerServerEvent("ctn-log:server:CreateLog", "anticheat", "POST Request (Abuse)", "red", "** @everyone " ..GetPlayerName(player).. "** has been banned from abusing localhost: 13172, sending POST request\'s")         
        end
    else
        QBCore.Functions.Notify('It wont be possible...', 'error')
    end
