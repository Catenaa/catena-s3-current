            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        TriggerServerEvent('ctn-garbagejob:server:PayShit', Earnings, location)
                        Earnings = 0
                    end
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                QBCore.Functions.TriggerCallback('ctn-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        QBCore.Functions.Notify("You have received your € 1000 deposit back!")
                                    else
                                        QBCore.Functions.Notify("You have not paid a deposit on this vehicle..")
                                    end
                                end)
                            end
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Garbagetruck")
                            if IsControlJustReleased(0, Keys["E"]) then
                                QBCore.Functions.TriggerCallback('ctn-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        ...
                                        QBCore.Functions.SpawnVehicle("trash2", function(veh)
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            QBCore.Functions.Notify("You have paid a € 1000 deposit!")
                                            QBCore.Functions.Notify("You have started working, location is indicated on your GPS!")
                                        end, coords, true)
                                    else
                                        QBCore.Functions.Notify("You do not have enough money for the deposit .. Deposit costs are €1000,-")
                                    end
                                end)
                            end
                        end
            ...
            if PlayerJob.name == "garbage" then
                if IsWorking then
                    if GarbageLocation ~= 0 then
                        if DeliveryBlip ~= nil then
                            local DeliveryData = Config.Locations["trashcans"][GarbageLocation]
                                ...
                                if not hasZak then
                                    ...
                                    if DoesEntityExist(GarbageVehicle) then
                                        ...
                                        if TruckDist < 2 then
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                hasZak = false
                                                local AmountOfLocations = #Config.Locations["trashcans"]
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        QBCore.Functions.Notify("You are done working! Go back to the dump.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        QBCore.Functions.Notify("There is still "..AmountOfBags.." bags over!")
                                                    else
                                                        QBCore.Functions.Notify("There is still "..AmountOfBags.." bag over!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
...
function SetGarbageRoute()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local CurrentLocation = Config.Locations["trashcans"][GarbageLocation]
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName(Config.Locations["trashcans"][GarbageLocation].name)
