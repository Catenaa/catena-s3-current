	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				CashoutCore.Functions.Progressbar("search_register", "Picking up Acid..", 5000, false, true, {
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				--PROP_HUMAN_BUM_BIN animazione
				--prop_cs_cardbox_01 oggetto di spawn  prop_plant_01a
				CashoutCore.Functions.Progressbar("search_register", "Picking Sulfuric..", 5000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
...
function Processweed3d()  -- simple animations to loop while process is taking place
	...
	CashoutCore.Functions.Progressbar("search_register", "Trying to Process..", 30000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
...
function SellDrug3()  -- simple animations to loop while process is taking place
	...
	CashoutCore.Functions.Progressbar("search_register", "Trying to Process..", 5000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	  ...
	  TriggerServerEvent('xd_drugs_weed:sellmeth')
		local timeLeft = Config.Delays.WeedProcessing / 5000
...
function procOn()
    ...
    ----
    if(count == 0) then
    CashoutCore.Functions.Progressbar("search_register", "Pikup sodium hydroxide", 1000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        disableInventory = true,
