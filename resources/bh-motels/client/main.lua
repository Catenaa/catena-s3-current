    ...
    if motelDist < 200 then
     ...
     if entryDist < 1.2 then
      if rid.owner == PlayerId then
       ...
       if rid.lock then
        exports['mythic_notify']:SendAlert('error', 'Room Locked')
       else
        TriggerServerEvent('hotel:createRoom', {id = rid.roomno, x = rid.entry.x, y = rid.entry.y, z = rid.entry.z-depthZ, outZ = rid.entry.outZ})
        showmenushit = false
        roomInfo = rid
      ...
      exports['mythic_notify']:SendAlert('error', 'Rak b3id 3la room')
     end
    end
   end
  end
 else
  exports['mythic_notify']:SendAlert('inform', 'Please specify a room number')
 end
end)
...
RegisterCommand('breach', function(source, args)
 ...
 if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
  ...
  if not found then
   exports['mythic_notify']:SendAlert('error', 'Hta had ma kra had room 🚫')
  end
 else
  exports['mythic_notify']:SendAlert('error', 'nta machi polici 🚫')
 end
end)
...
function OpenCupboard(room)
    TriggerEvent('bh-inventoryhud:openInventory', {
        type = 'cupboard',
        owner = room,
        take = true,
        store = true,
 ...
 while true do
     ...
     if IsControlJustPressed(0, 38) then
        ...
        ESX.TriggerServerCallback('motels:mycash', function(cash)
         if cash >= complex.price then
          ...
          TriggerServerEvent('pw-motels:rentRoom', testHotel.roomno, testHotel.motelid)
            ...
            exports['mythic_notify']:SendAlert('error', 'ma3ndkch flouss fl banka 🚫')
         end
        end)
       else
        exports['mythic_notify']:SendAlert('error', 'Mba9awch les room 🚫')
       end
      end
      ...
      if ownsMotel then
       if Config.Complexs[i].name == Config.Complexs[myMotel.motel].name then
        ...
        exports['mythic_notify']:SendAlert('inform', '3andk deja room f '..Config.Complexs[tonumber(myMotel.motel)].name)
       end
      end
     end
    end
   ...
   if entryDist < 1.5 then
    if roomid.owner == PlayerId then
     if roomid.lock then
      ...
      if IsControlJustReleased(0,  Keys['G']) then
       exports['mythic_notify']:SendAlert('success', 'Locked ! 👍')
       TriggerEvent('dooranim')
  ...
  if showmenushit then
   ...
   if clothDist < 1.2 and showClothing then
     DrawText3D(myRoom.x - 2, myRoom.y + 2.5, myRoom.z + 1.25, "~s~[~g~E~s~] hel Lplacard")
     if IsControlJustReleased(0, 38) then
      ClothingMenu(roomInfo.ident, roomInfo.motelid, roomInfo.roomno)
   end
 end
            -- Stash Bed
   ...
   if stashDist < 1.2 then
    DrawText3D(myRoom.x-1.6, myRoom.y+1.2, myRoom.z+1.25, "~s~[~g~E~s~] access stash")
    if IsControlJustReleased(0, 38) then
...
if not cancelled then
   ...
   if stashDist2 < 2.5 then
    DrawText3D(myRoom.x+1.6, myRoom.y+0.3, myRoom.z+0.25, "~s~[~g~E~s~] access bed stash")
    if IsControlJustReleased(0, 38) then
      local playerPed = GetPlayerPed(-1)
         ESX.Streaming.RequestAnimDict('mini@repair', function()
             TaskPlayAnim(playerPed, 'mini@repair', 'fixing_a_player', 8.0, -8, -1, 49, 0, 0, 0, 0)
...
if not cancelled then
  OpenCupboard(roomInfo.ident, roomInfo.motelid, roomInfo.roomno)
  TriggerEvent('InteractSound_CL:PlayOnOne', 'zip', 0.8)
  end
end)
    end
...
function buildHotel(generator)
 ...
 exports['mythic_notify']:SendAlert('success', 'Tsayn chwiya !')
