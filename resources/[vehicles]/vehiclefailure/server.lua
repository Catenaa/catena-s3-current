...
QBCore.Functions.CreateUseableItem("mcrepairkit", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("vehiclefailure:client:RepairVehicleFull", source)
    end
...
RegisterServerEvent('ctn-vehiclefailure:removeItem')
AddEventHandler('ctn-vehiclefailure:removeItem', function(item)
    local src = source
