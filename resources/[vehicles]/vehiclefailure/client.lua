...
function RepairVehicleFull(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Tinkering ..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		QBCore.Functions.Notify("Vehicle fixed!")
		...
		TriggerServerEvent('ctn-vehiclefailure:removeItem', "advancedrepairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		QBCore.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
function RepairVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Tinkering ..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('ctn-vehiclefailure:removeItem', "repairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		QBCore.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		notification("~y~You must be in a vehicle to fix it!")
	end
end)
...
notification("~r~Er roadside assistance is available, call it via your phone!")
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								--exports['ctn-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['ctn-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
							end
...
function DamageRandomComponent()
	...
	--exports['ctn-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent, exports['ctn-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent) - randomDamage)
end
