...
AddEventHandler('playerDropped', function(reason) 
	...
	TriggerEvent("ctn-logs:server:CreateLog", "joinleave", "Dropped", "red", "**".. GetPlayerName(src) .. "** ("..GetPlayerIdentifiers(src)[1]..") left..")
	TriggerEvent("ctn-logs:server:sendLog", GetPlayerIdentifiers(src)[1], "joined", {})
...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	if not (QBCore.Functions.IsWhitelisted(src)) then
		QBCore.Functions.Kick(src, 'You are not whitelisted.', setKickReason, deferrals)
        CancelEvent()
		return false
	end
	...
	TriggerEvent("ctn-queue:playerConnecting", src, setKickReason, deferrals)
end)
...
AddEventHandler('QBCore:server:CloseServer', function(reason)
    ...
    if QBCore.Functions.HasPermission(source, "admin") or QBCore.Functions.HasPermission(source, "god") then 
		...
		QBCore.Functions.Kick(src, "You have no permission for loser here..", nil, nil)
    end
end)
...
RegisterServerEvent("QBCore:ReceivedSalary")
AddEventHandler('QBCore:ReceivedSalary', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	
	if Player ~= nil then
		Player.Functions.AddMoney("bank", Player.PlayerData.job.payment, 'salary-update')
		TriggerClientEvent('QBCore:Notify', src, "You received your salary. [€"..Player.PlayerData.job.payment .. "]")
	end
end)
...
AddEventHandler('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, "You went off duty!")
		TriggerClientEvent("debug", src, 'QBCore: You went off duty!', 'error')
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, "You went on duty!")
		TriggerClientEvent("debug", src, 'QBCore: You went on duty!', 'success')
...
RegisterServerEvent('QBCore:HasItem')
AddEventHandler('QBCore:HasItem', function(command, args)
	local retval = false
	local Player = QBCore.Functions.GetPlayer(source)
	if Player ~= nil then 
		if Player.Functions.GetItemByName(itemName) ~= nil then
...
QBCore.Functions.CreateCallback('QBCore:HasMoney', function(source, cb, moneyType, Count, Remove)
	local retval = false
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player ~= nil then 
...
AddEventHandler('QBCore:Command:CheckOwnedVehicle', function(VehiclePlate)
	if VehiclePlate ~= nil then
		QBCore.Functions.ExecuteSql(false, "SELECT * FROM `ctnvehicles` WHERE `plate` = '"..VehiclePlate.."'", function(result)
			if result[1] ~= nil then
				QBCore.Functions.ExecuteSql(false, "UPDATE `ctnvehicles` SET `state` = '1' WHERE `citizenid` = '"..result[1].citizenid.."'")
				TriggerEvent('ctn-garages:server:RemoveVehicle', result[1].citizenid, VehiclePlate)
			end
		end)
	end
end)