...
QBCore.Functions.IsWhitelisted = function(source)
	local rtn = nil
	local discord = nil
	local role = QBConfig.Roleid
	local roleadmin = QBConfig.Roleidadmin
	...
	if discord then
		PerformHttpRequest("https://discordapp.com/api/guilds/"..QBConfig.Guildid.."/members/"..string.sub(discord, 9), function(err, text, headers)
			if err == 200 then
				local data = json.decode(text)
				for k,v in pairs(data['roles']) do
					if v == role and QBConfig.Server.Opened or v == roleadmin then
						rtn = true
					end
				end
		...
		end, "GET", "", {["Content-type"] = "application/json", ["Authorization"] = "Bot "..QBConfig.Bottoken})
	else
		rtn = false
	end
