...
RegisterNetEvent('ctn:Client:TakeHostage')
AddEventHandler('ctn:Client:TakeHostage', function()
    takeHostage()
end)
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		if NoWeapon then
			QBCore.Functions.Notify("You can't hostage anyone with this weapon")
			return
		end
		...
		QBCore.Functions.Notify("There's no one around")
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[G] Release hostage. [H] Shoot hostage.")
