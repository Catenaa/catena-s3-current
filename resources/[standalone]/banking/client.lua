...
RegisterNetEvent('ctn-banking:client:SetBankClosed')
AddEventHandler('ctn-banking:client:SetBankClosed', function(BankId, bool)
    ...
    while true do
        ...
        if nearbank then
          ...
          if not banks[bankkey].Closed then
            ...
            DrawText3Ds(banks[bankkey].x, banks[bankkey].y, banks[bankkey].z + 0.3, '[E] Use Banking')
        ...
        elseif IsNearATM() then
          atBank = true
          inRange = true
          DrawText3Ds(pos.x, pos.y, pos.z, '[E] Use Banking')
          if IsControlJustPressed(1, Keys["E"])  then
              if (not IsInVehicle()) then
                  if bankOpen then
                      closeGui()
...
AddEventHandler('banking:client:CheckDistance', function(targetId, amount)
  ...
  if player ~= -1 and distance < 2.5 then
    ...
    QBCore.Functions.Notify('Youre not near the person..', 'error')
  end
end)