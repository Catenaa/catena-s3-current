			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Houses[ClosestHouse].coords.enter.x, Houses[ClosestHouse].coords.enter.y, Houses[ClosestHouse].coords.enter.z, true) < 2.5 then 
				if not Houses[ClosestHouse].locked or HasKey(ClosestHouse) then 
					if HasKey(ClosestHouse) and not Houses[ClosestHouse].locked then 
						DrawText3D(Houses[ClosestHouse].coords.enter.x, Houses[ClosestHouse].coords.enter.y, Houses[ClosestHouse].coords.enter.z, "[~g~E~w~] to go in / [~g~L~w~] to lock the door")
					elseif HasKey(ClosestHouse) and Houses[ClosestHouse].locked then 
						DrawText3D(Houses[ClosestHouse].coords.enter.x, Houses[ClosestHouse].coords.enter.y, Houses[ClosestHouse].coords.enter.z, "[~g~E~w~] to go in / [~g~L~w~] to open the door")
					else 
						DrawText3D(Houses[ClosestHouse].coords.enter.x, Houses[ClosestHouse].coords.enter.y, Houses[ClosestHouse].coords.enter.z, "[~g~E~w~] to go in")
					end
		...
		elseif InHouse ~= nil and not DecoMode then
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Houses[InHouse].coords.exit.x, Houses[InHouse].coords.exit.y, Houses[InHouse].coords.exit.z, true) < 2.5 then 
				if HasKey(InHouse) and not Houses[InHouse].locked then 
					DrawText3D(Houses[InHouse].coords.exit.x, Houses[InHouse].coords.exit.y, Houses[InHouse].coords.exit.z, "[~g~E~w~] to go out / [~g~L~w~] to lock door")
				elseif HasKey(InHouse) and Houses[InHouse].locked then 
					DrawText3D(Houses[InHouse].coords.exit.x, Houses[InHouse].coords.exit.y, Houses[InHouse].coords.exit.z, "[~g~E~w~] to go out / [~g~L~w~] to open door")
				else 
					DrawText3D(Houses[InHouse].coords.exit.x, Houses[InHouse].coords.exit.y, Houses[InHouse].coords.exit.z, "[~g~E~w~] to go out")
				end
				if IsControlJustReleased(0, Keys["E"]) then 
					TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 1.0)
					DoScreenFadeOut(500)
